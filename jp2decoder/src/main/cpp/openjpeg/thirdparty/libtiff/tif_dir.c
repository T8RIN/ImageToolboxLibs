    ...
    switch (standard_tag) {
        ...
        case TIFFTAG_SMINSAMPLEVALUE:
            if (tif->tif_flags & TIFF_PERSAMPLE)
                _TIFFsetDoubleArray(&td->td_sminsamplevalue, va_arg(ap,
            double*), td->td_samplesperpixel);
            else
            setDoubleArrayOneValue(&td->td_sminsamplevalue, va_arg(ap,
            double), td->td_samplesperpixel);
            break;
        case TIFFTAG_SMAXSAMPLEVALUE:
            if (tif->tif_flags & TIFF_PERSAMPLE)
                _TIFFsetDoubleArray(&td->td_smaxsamplevalue, va_arg(ap,
            double*), td->td_samplesperpixel);
            else
            setDoubleArrayOneValue(&td->td_smaxsamplevalue, va_arg(ap,
            double), td->td_samplesperpixel);
            break;
        ...
        case TIFFTAG_REFERENCEBLACKWHITE:
            ...
            _TIFFsetFloatArray(&td->td_refblackwhite, va_arg(ap,
            float*), 6);
            break;
        ...
        default: {
            ...
            if (fip->field_type == TIFF_ASCII) {
                ...
                if (fip->field_tag == TIFFTAG_DOTRANGE
                    ...
                    uint16 v[2];
                    v[0] = (uint16) va_arg(ap,
                    int);
                    v[1] = (uint16) va_arg(ap,
                    int);
...
TIFFAdvanceDirectory(TIFF *tif, uint64 *nextdir, uint64 *off) {
    ...
    if (isMapped(tif)) {
        ...
        if (!(tif->tif_flags & TIFF_BIGTIFF)) {
            ...
            if (((uint64) poffa != poff) || (poffb < poffa) ||
                (poffb < (tmsize_t) sizeof(uint16)) || (poffb > tif->tif_size)) {
            ...
            *nextdir = nextdir32;
            ...
            if (((uint64) poffa != poff) || (poffb < poffa) ||
                (poffb < (tmsize_t) sizeof(uint64)) || (poffb > tif->tif_size)) {
                TIFFErrorExt(tif->tif_clientdata, module, "Error fetching directory count");
                return (0);
            }
            ...
            if ((poffc < poffb) || (poffc < dircount16 * 20) || (poffd < poffc) ||
