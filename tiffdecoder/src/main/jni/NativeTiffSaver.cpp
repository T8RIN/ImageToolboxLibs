...
JNIEXPORT jboolean JNICALL Java_org_beyka_tiffbitmapfactory_TiffSaver_save
        (JNIEnv *env, jclass clazz, jstring filePath, jint fileDescriptor, jobject bitmap,
         jobject options, jboolean append) {
...
*/
    ...
    jfieldID gOptions_authorFieldID = env->GetFieldID(jSaveOptionsClass, "author",
                                                      "Ljava/lang/String;");
    ...
    jfieldID gOptions_copyrightFieldID = env->GetFieldID(jSaveOptionsClass, "copyright",
                                                         "Ljava/lang/String;");
    ...
    jfieldID gOptions_imgDescrFieldID = env->GetFieldID(jSaveOptionsClass, "imageDescription",
                                                        "Ljava/lang/String;");
    ...
    jfieldID softwareNameFieldID = env->GetStaticFieldID(jBuildConfigClass, "softwarename",
                                                         "Ljava/lang/String;");
    jstring jsoftwarename = (jstring) env->GetStaticObjectField(jBuildConfigClass,
                                                                softwareNameFieldID);
...
*/
    ...
    if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
        ...
        for (int i = 0; i < img_height; i++) {
            TIFFWriteEncodedStrip(output_image, i, &bilevel[i * compressedWidth],
                                  (compressedWidth));
        }
        free(bilevel);
        ...
        TIFFSetField(output_image, TIFFTAG_ROWSPERSTRIP, 1);
        for (int row = 0; row < img_height; row++) {
            TIFFWriteEncodedStrip(output_image, row, &img[row * img_width],
                                  img_width * sizeof(uint32));
            //TIFFWriteScanline(output_image, &img[row * img_width], row, 0);
        }
    }
...
unsigned char *convertArgbToBilevel(uint32 *source, jint width, jint height) {
    ...
    for (int j = 0; j < height; j++) {
        ...
        for (int i = 0; i < width; i++) {
            ...
            grayPix = (0.2125 * (colorMask & crPix >> 16) + 0.7154 * (colorMask & crPix >> 8) +
                       0.0721 * (colorMask & crPix));
