...
static int
readSeparateStripsIntoBuffer(TIFF *, uint8 *, uint32, uint32, tsample_t, struct dump_opts *);

static int
readContigTilesIntoBuffer(TIFF *, uint8 *, uint32, uint32, uint32, uint32, tsample_t, uint16);

static int
readSeparateTilesIntoBuffer(TIFF *, uint8 *, uint32, uint32, uint32, uint32, tsample_t, uint16);
...
static int
writeBufferToSeparateStrips(TIFF *, uint8 *, uint32, uint32, tsample_t, struct dump_opts *);

static int
writeBufferToSeparateTiles(TIFF *, uint8 *, uint32, uint32, tsample_t, struct dump_opts *);
       ...
       */
            ...
            if (((bps % 8) == 0) && (count == spp)) {
                ...
                for (trow = 0; trow < nrow; trow++) {
                    ...
                    switch (shift_width) {
                        case 0:
                            if (extractContigSamplesBytes(src, dst, ncol, sample,
                        ...
                        case 1:
                            if (bps == 1) {
                                ...
                                break;
                            } else if (extractContigSamplesShifted16bits(src, dst, ncol,
                                                                         sample, spp,
                                                                         bps, count,
                                                                         0, ncol,
                                                                         prev_trailing_bits)) {
                            ...
                            break;
                        case 2:
                            if (extractContigSamplesShifted24bits(src, dst, ncol,
                                                                  sample, spp,
                                                                  bps, count,
                                                                  0, ncol,
                                                                  prev_trailing_bits)) {
                        ...
                        case 4:
                        case 5:
                            if (extractContigSamplesShifted32bits(src, dst, ncol,
                                                                  sample, spp,
                                                                  bps, count,
                                                                  0, ncol,
                                                                  prev_trailing_bits)) {
                            ...
                            break;
                        default:
                            TIFFError("readContigTilesIntoBuffer", "Unsupported bit depth %d", bps);
                            return 1;
                    }
                }
...
static int readSeparateTilesIntoBuffer(TIFF *in, uint8 *obuf,
   ...
   */
    for (row = 0; row < imagelength; row += tl) {
        nrow = (row + tl > imagelength) ? imagelength - row : tl;
        for (col = 0; col < imagewidth; col += tw) {
            for (s = 0; s < spp; s++) {  /* Read each plane of a tile set into srcbuffs[s] */
        ...
        switch (c) {
            ...
            case 'd':
                ...
                *dirnum = start - 1;
                break;
            case 'e':
                switch (tolower(optarg[0])) /* image export modes*/
                {
                    case 'c':
            ...
            case 't':    /* generate tiled output */
                ...
                break;
            case 'v':
                TIFFError("Library Release", "%s", TIFFGetVersion());
		    ...
		    */
                    ...
                    *opt_offset = '\0';
                    ...
                    } else { /* Look for dump level specification */
            ...
            case 'E':    /* edge reference */
                switch (tolower(optarg[0])) {
                    ...
                    case 'r':
                        ...
                        break;
                    default:
                        TIFFError("Edge reference must be top, bottom, left, or right", "%s",
                                  optarg);
                        TIFFError("For valid options type", "tiffcrop -h");
                        exit(-1);
                }
            ...
            case 'N':    /* list of images to process */
                for (i = 0, opt_ptr = strtok(optarg, ",");
                     ...
                     (opt_ptr = strtok(NULL, ","))) { /* We do not know how many images are in file yet
			* so we build a list to include the maximum allowed
            ...
            case 'O': /* page orientation */
                switch (tolower(optarg[0])) {
                    ...
                    case 'l':
                        ...
                        break;
                    default:
                        TIFFError("Orientation must be portrait, landscape, or auto.", "%s",
                                  optarg);
                        TIFFError("For valid options type", "tiffcrop -h");
                        exit(-1);
                }
            ...
            case 'R': /* rotate image or cropped segment */
                ...
                switch (strtoul(optarg, NULL, 0)) {
                    ...
                    case 270:
                        ...
                        break;
                    default:
                        TIFFError("Rotation must be 90, 180, or 270 degrees clockwise", "%s",
                                  optarg);
                        TIFFError("For valid options type", "tiffcrop -h");
                        exit(-1);
                }
            ...
            case 'S':    /* subdivide into Cols:Rows sections, eg 3:2 would be 3 across and 2 down */
                ...
                if ((page->cols * page->rows) > MAX_SECTIONS) {
                    TIFFError("Limit for subdivisions, ie rows x columns, exceeded", "%d",
                              MAX_SECTIONS);
                    exit(-1);
                }
...
update_output_file(TIFF **tiffout, char *mode, int autoindex,
    ...
    {
        if (autoindex) { /* create a new filename for each export */
            findex++;
...
main(int argc, char *argv[]) {
    ...
    while (optind < argc - 1) {
       ...
       */
            ...
            if (page.mode ==
                PAGE_MODE_NONE) {  /* Whole image or sections not based on output page size */
                if (crop.selections > 0) {
                ...
                } else  /* One file all images and sections */
                {
                    ...
                    if (writeCroppedImage(in, out, &image, &dump, crop.combined_width,
                                          crop.combined_length, crop_buff, next_page,
                                          total_pages)) {
         ...
         */
                ...
                if (writeImageSections(in, out, &image, &page, sections, &dump, sect_src,
                                       &sect_buff)) {
...
/* Debugging functions */
static int
dump_data(FILE *dumpfile, int format, char *dump_tag, unsigned char *data, uint32 count) {
    int j, k;
    uint32 i;
    char dump_array[10];
...
extractContigSamples8bits(uint8 *in, uint8 *out, uint32 cols,
    ...
    buff1 = buff2 = 0;
    for (col = start; col < end; col++) {    /* Compute src byte(s) and bits within byte(s) */
        bit_offset = col * bps * spp;
        for (sindex = sample; (sindex < spp) && (sindex < (sample + count)); sindex++) {
            if (sindex == 0) {
                src_byte = bit_offset / 8;
...
extractContigSamples32bits(uint8 *in, uint8 *out, uint32 cols,
    ...
    for (col = start; col < end; col++) {
        ...
        for (sindex = sample; (sindex < spp) && (sindex < (sample + count)); sindex++) {
            ...
            } else { /* add another bps bits to the buffer */
...
extractContigSamplesShifted32bits(uint8 *in, uint8 *out, uint32 cols,
    ...
    for (col = start; col < end; col++) {
        ...
        for (sindex = sample; (sindex < spp) && (sindex < (sample + count)); sindex++) {
            ...
            if (ready_bits < 32) { /* add another bps bits to the buffer */
                bytebuff1 = bytebuff2 = bytebuff3 = bytebuff4 = 0;
                buff2 = (buff2 | (buff1 >> ready_bits));
            } else  /* If we have a full buffer's worth, write it out */
            {
...
extractContigSamplesToBuffer(uint8 *out, uint8 *in, uint32 rows, uint32 cols,
    ...
    for (row = 0; row < rows; row++) {
        ...
        switch (shift_width) {
            case 0:
                if (extractContigSamplesBytes(src, dst, cols, sample,
            ...
            case 1:
                if (bps == 1) {
                    if (extractContigSamples8bits(src, dst, cols, sample,
                        ...
                        return (1);
                    break;
                } else if (extractContigSamples16bits(src, dst, cols, sample,
                                                      spp, bps, count, first_col, cols))
                    return (1);
                break;
            case 2:
                if (extractContigSamples24bits(src, dst, cols, sample,
                                               spp, bps, count, first_col, cols))
                    return (1);
                break;
            case 3:
            case 4:
            case 5:
                if (extractContigSamples32bits(src, dst, cols, sample,
                                               spp, bps, count, first_col, cols))
                    return (1);
                break;
            default:
                TIFFError("extractContigSamplesToBuffer", "Unsupported bit depth: %d", bps);
                return (1);
        }
        if ((dump->outfile != NULL) && (dump->level == 4))
    ...
    for (row = 0; row < rows; row++) {
        ...
        switch (shift_width) {
            ...
            case 5:
                ...
                break;
            default:
                TIFFError("extractContigSamplesToTileBuffer", "Unsupported bit depth: %d", bps);
                return (1);
        }
        if ((dump->outfile != NULL) && (dump->level == 4))
            dump_buffer(dump->outfile, dump->format, 1, dst_rowsize, row, dst);
   ...
   */
    if (crop->crop_mode & CROP_MARGINS) {
        if (crop->res_unit != RESUNIT_INCH && crop->res_unit !=
                                              RESUNIT_CENTIMETER) { /* User has specified pixels as reference unit */
...
getCropOffsets(struct image_data *image, struct crop_mask *crop, struct dump_opts *dump) {
    ...
    if ((crop->crop_mode & CROP_MARGINS) ||
        ...
        if (crop->crop_mode & CROP_REGIONS)
            return (0);
    } else { /* cropped area is the full image */
        offsets.tmargin = 0;
        offsets.lmargin = 0;
        offsets.bmargin = 0;
        offsets.rmargin = 0;
        ...
        switch (crop->edge_ref) {
            ...
            default:
                ...
                crop->regionlist[i].y1 =
                        offsets.starty + (uint32)(offsets.crop_length * 1.0 * (seg - 1) / total);
...
computeOutputPixelOffsets(struct crop_mask *crop, struct image_data *image,
   ...
   */
    ...
    if (page->mode & PAGE_MODE_MARGINS) {
        if (page->res_unit == RESUNIT_INCH ||
            page->res_unit == RESUNIT_CENTIMETER) { /* inches or centimeters specified */
            hmargin = (uint32)(page->hmargin * scale * page->hres * ((image->bps + 7) / 8));
            vmargin = (uint32)(page->vmargin * scale * page->vres * ((image->bps + 7) / 8));
        } else { /* Otherwise user has specified pixels as reference unit */
    ...
    switch (readunit) {
        ...
        case TILE:
            if (planar == PLANARCONFIG_CONTIG) {
                ...
                if (!(readSeparateTilesIntoBuffer(in, read_buff, length, width, tw, tl, spp,
                                                  bps))) {
        ...
        switch (crop->edge_ref) {
            ...
            case EDGE_BOTTOM:
                ...
                for (row = first_row; row <= last_row; row++) {
                    ...
                    switch (shift_width) {
                        case 0:
                            if (extractContigSamplesBytes(src, dst, img_width, sample,
                        ...
                        case 1:
                            if (bps == 1) {
                                ...
                                break;
                            } else if (extractContigSamplesShifted16bits(src, dst, img_width,
                                                                         sample, spp, bps, count,
                                                                         first_col, last_col + 1,
                                                                         prev_trailing_bits)) {
                                TIFFError("extractCompositeRegions",
                            ...
                            break;
                        case 2:
                            if (extractContigSamplesShifted24bits(src, dst, img_width,
                                                                  sample, spp, bps, count,
                                                                  first_col, last_col + 1,
                                                                  prev_trailing_bits)) {
                                TIFFError("extractCompositeRegions",
                        ...
                        case 4:
                        case 5:
                            if (extractContigSamplesShifted32bits(src, dst, img_width,
                                                                  sample, spp, bps, count,
                                                                  first_col, last_col + 1,
                                                                  prev_trailing_bits)) {
                                TIFFError("extractCompositeRegions",
                            ...
                            break;
                        default:
                            TIFFError("extractCompositeRegions", "Unsupported bit depth %d", bps);
                            return (1);
                    }
                }
            ...
            case EDGE_RIGHT:
                ...
                for (row = first_row; row <= last_row; row++) {
                    ...
                    switch (shift_width) {
                        case 0:
                            if (extractContigSamplesBytes(src, dst, img_width,
                                                          sample, spp, bps, count,
                                                          first_col, last_col + 1)) {
                                TIFFError("extractCompositeRegions",
                                          "Unable to extract row %d", row);
    ...
    for (row = first_row; row <= last_row; row++) {
        ...
        switch (shift_width) {
            ...
            case 5:
                ...
                break;
            default:
                TIFFError("extractSeparateRegion", "Unsupported bit depth %d", bps);
                return (1);
        }
    }
...
#endif
                    if (shift2 > shift1) {
                        bytebuff1 = src_buff[offset1 + full_bytes] &
...
writeSingleSection(TIFF *in, TIFF *out, struct image_data *image,
   ...
   */
	...
	*/
    ...
    else CopyField(TIFFTAG_PLANARCONFIG, config);
    if (spp <= 4)
...
/* SMinSampleValue & SMaxSampleValue */
     ...
     */
        ...
        case COMPRESSION_DEFLATE:
            ...
            else CopyField(TIFFTAG_PREDICTOR, predictor);
            break;
        ...
        case COMPRESSION_CCITTFAX4:
            if (compression == COMPRESSION_CCITTFAX3) {
                ...
                else CopyField(TIFFTAG_GROUP3OPTIONS, g3opts);
            } else
    ...
    } else  /* Separated Images */
    {
        ...
        for (i = 0; i < crop->selections; i++) {
            ...
            if (extractSeparateRegion(image, crop, read_buff, crop_buff, i)) {
                TIFFError("processCropSelections", "Unable to extract cropped region %d from image",
                          i);
                return (-1);
            }
        ...
        for (sample = 0; sample < spp; sample++) {
            ...
            switch (rotation) {
                ...
                case 270:
                    ...
                    break;
                default:
                    TIFFError("rotateContigSamples8bits", "Invalid rotation %d", rotation);
                    return (1);
            }
            matchbits = maskbits << (8 - src_bit - bps);
...
rotateImage(uint16 rotation, struct image_data *image, uint32 *img_width,
    ...
    switch (rotation) {
        ...
        case 270:
            break;
        default:
            TIFFError("rotateImage", "Invalid rotation angle %d", rotation);
            return (-1);
    }
    ...
    switch (rotation) {
        case 180:
            ...
            } else { /* non 8 bit per sample data */
                for (row = 0; row < length; row++) {
                    ...
                    switch (shift_width) {
                        ...
                        case 5:
                            ...
                            break;
                        default:
                            TIFFError("rotateImage", "Unsupported bit depth %d", bps);
                            _TIFFfree(rbuff);
                            return (-1);
                    }
                }
                ...
                for (col = 0; col < width; col++) {
                    ...
                    switch (shift_width) {
                        case 1:
                            ...
                            break;
                        case 2:
                            if (rotateContigSamples24bits(rotation, spp, bps, width,
                                                          length, col, src, dst)) {
                                _TIFFfree(rbuff);
                                return (-1);
                            }
                        ...
                        case 4:
                        case 5:
                            if (rotateContigSamples32bits(rotation, spp, bps, width,
                                                          length, col, src, dst)) {
                                _TIFFfree(rbuff);
                                return (-1);
                            }
...
reverseSamplesBytes(uint16 spp, uint16 bps, uint32 width,
    ...
    switch (bps / 8) {
        ...
        case 1: /* Use byte copy only for single byte per sample data */
            ...
            break;
        default:
            TIFFError("reverseSamplesBytes", "Unsupported bit depth %d", bps);
            return (1);
    }
    return (0);
...
static int
mirrorImage(uint16 spp, uint16 bps, uint16 mirror, uint32 width, uint32 length,
            unsigned char *ibuff) {
    int shift_width;
    uint32 bytes_per_pixel, bytes_per_sample;
    ...
    switch (mirror) {
        ...
        case MIRROR_VERT:
            ...
            if (line_buff == NULL) {
                TIFFError("mirrorImage", "Unable to allocate mirror line buffer of %1u bytes",
                          rowsize);
                return (-1);
            }
        ...
        case MIRROR_HORIZ :
            if ((bps % 8) == 0) /* byte alligned data */
            {
                ...
                for (row = 0; row < length; row++) {
                    ...
                    switch (shift_width) {
                        case 1:
                            if (reverseSamples16bits(spp, bps, width, src, line_buff)) {
                                _TIFFfree(line_buff);
                                return (-1);
                            }
                            ...
                            break;
                        case 2:
                            if (reverseSamples24bits(spp, bps, width, src, line_buff)) {
                                _TIFFfree(line_buff);
                                return (-1);
                            }
                            _TIFFmemcpy(src, line_buff, rowsize);
                        ...
                        case 4:
                        case 5:
                            if (reverseSamples32bits(spp, bps, width, src, line_buff)) {
                                _TIFFfree(line_buff);
                                return (-1);
                            }
                            _TIFFmemcpy(src, line_buff, rowsize);
                            break;
                        default:
                            TIFFError("mirrorImage", "Unsupported bit depth %d", bps);
                            _TIFFfree(line_buff);
                            return (-1);
                    }
                }
        ...
        default:
            TIFFError("mirrorImage", "Invalid mirror axis %d", mirror);
            return (-1);
            break;
    }
...
static int
invertImage(uint16 photometric, uint16 spp, uint16 bps, uint32 width, uint32 length,
            unsigned char *work_buff) {
    uint32 row, col;
