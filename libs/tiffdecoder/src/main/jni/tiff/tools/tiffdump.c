...
PrintByte(FILE *fd, const char *fmt, TIFFDirEntry *dp) {
    ...
    if (hdr.tiff_magic == TIFF_BIGENDIAN) {
        switch ((int) dp->tdir_count) {
            case 4:
                fprintf(fd, fmt, sep, dp->tdir_offset & 0xff);
                sep = " ";
            case 3:
                fprintf(fd, fmt, sep, (dp->tdir_offset >> 8) & 0xff);
                sep = " ";
            case 2:
                fprintf(fd, fmt, sep, (dp->tdir_offset >> 16) & 0xff);
                sep = " ";
            case 1:
                fprintf(fd, fmt, sep, dp->tdir_offset >> 24);
        }
    } else {
        switch ((int) dp->tdir_count) {
            case 4:
...
PrintShort(FILE *fd, const char *fmt, TIFFDirEntry *dp) {
    ...
    if (hdr.tiff_magic == TIFF_BIGENDIAN) {
        switch (dp->tdir_count) {
            case 2:
                fprintf(fd, fmt, sep, dp->tdir_offset & 0xffff);
                sep = " ";
            case 1:
                fprintf(fd, fmt, sep, dp->tdir_offset >> 16);
        }
    } else {
        switch (dp->tdir_count) {
            case 2:
