...
BaseTiffConverter::BaseTiffConverter(JNIEnv *e, jclass clazz, jstring in, jstring out, jobject opts,
    ...
    jConvertOptionsClass = env->FindClass(
            "org/beyka/tiffbitmapfactory/TiffConverter$ConverterOptions");
...
BaseTiffConverter::BaseTiffConverter(JNIEnv *e, jclass clazz, jint in, jint out, jobject opts,
                                     jobject listener) {
    boundX = boundY = boundWidth = boundHeight = -1;
    hasBounds = 0;
    availableMemory = 8000 * 8000 * 4;
...
void BaseTiffConverter::readOptions() {
    ...
    if (ci == 1 || ci == 2 || ci == 3 || ci == 4 || ci == 5 || ci == 7 || ci == 8 || ci == 32773 ||
        ci == 32946)
        compressionInt = ci;
    else
        compressionInt = 5;
    ...
    jfieldID imgDescrFieldID = env->GetFieldID(jConvertOptionsClass, "imageDescription",
                                               "Ljava/lang/String;");
    ...
    jfieldID softwareFieldID = env->GetFieldID(jConvertOptionsClass, "software",
                                               "Ljava/lang/String;");
    ...
    jfieldID gOptions_xResolutionFieldID = env->GetFieldID(jConvertOptionsClass, "xResolution",
                                                           "F");
    xRes = env->GetFloatField(optionsObj, gOptions_xResolutionFieldID);
    jfieldID gOptions_yResolutionFieldID = env->GetFieldID(jConvertOptionsClass, "yResolution",
                                                           "F");
...
void
BaseTiffConverter::rotateTileLinesVertical(uint32 tileHeight, uint32 tileWidth, uint32 *whatRotate,
                                           uint32 *bufferLine) {
...
void BaseTiffConverter::rotateTileLinesHorizontal(uint32 tileHeight, uint32 tileWidth,
                                                  uint32 *whatRotate, uint32 *bufferLine) {
    uint32 buf;
...
void BaseTiffConverter::normalizeTile(uint32 tileHeight, uint32 tileWidth, uint32 *rasterTile) {
    ...
    if (sy != 0) {
        for (int y = 0; y < tileHeight - sy - 1; y++) {
            memcpy(rasterTile + (y * tileWidth), rasterTile + ((y + sy) * tileWidth),
                   tileWidth * sizeof(uint32));
        }
    }
    if (sx != 0) {
        for (int y = 0; y < tileHeight; y++) {
