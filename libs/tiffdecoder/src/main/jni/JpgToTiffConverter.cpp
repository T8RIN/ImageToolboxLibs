...
JpgToTiffConverter::JpgToTiffConverter(JNIEnv *e, jclass clazz, jstring in, jstring out,
                                       jobject opts, jobject listener)
...
JpgToTiffConverter::JpgToTiffConverter(JNIEnv *e, jclass clazz, jint in, jint out, jobject opts,
                                       jobject listener)
        : BaseTiffConverter(e, clazz, in, out, opts, listener) {
    jpeg_struct_init = 0;
...
jboolean JpgToTiffConverter::convert() {
    ...
    if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
        compressionInt == COMPRESSION_CCITTFAX4) {
    ...
    unsigned long MB2 = (availableMemory == -1 || availableMemory > 3 * 1024 * 1024) ? 2 * 1024 *
                                                                                       1024 :
                        width * 4;
    ...
    if (compressionInt == COMPRESSION_JPEG) {
        estimateMem += 0;
    } else if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
               compressionInt == COMPRESSION_CCITTFAX4) {
    ...
    if (compressionInt == COMPRESSION_JPEG) {
        ...
        while (cinfo.output_scanline < height) {
            ...
            if (rowCounter == rowPerStrip) {
                ...
                if (compressionInt == COMPRESSION_CCITTRLE ||
                    compressionInt == COMPRESSION_CCITTFAX3 ||
                    compressionInt == COMPRESSION_CCITTFAX4) {
                    int compressedWidth = (width / 8 + 0.5);
                    unsigned char *bilevel = convertArgbToBilevel(data, componentsPerPixel, width,
                                                                  rowPerStrip);
                    ret = TIFFWriteEncodedStrip(tiffImage, totalRowCounter / rowPerStrip - 1,
                                                bilevel, compressedWidth * sizeof(unsigned char) *
                                                         rowPerStrip);
                    free(bilevel);
                } else {
                    ret = TIFFWriteEncodedStrip(tiffImage, totalRowCounter / rowPerStrip - 1, data,
                                                rowPerStrip * rowSize);
                }
                rowCounter = 0;
                shouldWrite = false;
        ...
        if (shouldWrite) {
            ...
            if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
                ...
                unsigned char *bilevel = convertArgbToBilevel(data, componentsPerPixel, width,
                                                              rowPerStrip);
                ret = TIFFWriteEncodedStrip(tiffImage, totalRowCounter / rowPerStrip, bilevel,
                                            compressedWidth * sizeof(unsigned char) * rowPerStrip);
                free(bilevel);
            } else {
                ret = TIFFWriteEncodedStrip(tiffImage, totalRowCounter / rowPerStrip, data,
                                            rowPerStrip * rowSize);
            }
        }
        delete[] data;
...
unsigned char *
JpgToTiffConverter::convertArgbToBilevel(unsigned char *data, int components, uint32 width,
                                         uint32 height) {
    unsigned char red;
    unsigned char green;
    unsigned char blue;
