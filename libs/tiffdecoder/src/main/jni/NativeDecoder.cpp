...
jobject NativeDecoder::getBitmap() {
    ...
    jboolean inJustDecodeBounds = env->GetBooleanField(optionsObject,
...
//Read Config from options. Use ordinal field from ImageConfig class
    ...
    if (!hasBounds) {
        ...
        switch (getDecodeMethod()) {
            case DECODE_METHOD_IMAGE:
                raster = getSampledRasterFromImageWithBounds(inSampleSize, &newBitmapWidth,
                                                             &newBitmapHeight);
                break;
            case DECODE_METHOD_TILE:
                raster = getSampledRasterFromTileWithBounds(inSampleSize, &newBitmapWidth,
                                                            &newBitmapHeight);
                break;
            case DECODE_METHOD_STRIP:
                raster = getSampledRasterFromStripWithBounds(inSampleSize, &newBitmapWidth,
                                                             &newBitmapHeight);
                break;
        }
    ...
    *bitmapheight = origheight / inSampleSize;
    ...
    for (int i = 0; i < stripMax * rowPerStrip; i += rowPerStrip) {
        ...
        if (inSampleSize == 1) {
            ...
            if (isSecondRasterExist) {
                _TIFFmemcpy(matrixBottomLine,
                            rasterForBottomLine /*+ lineAddrToCopyBottomLine * origwidth*/,
                            sizeof(unsigned int) * origwidth);
            }
            int workWritedLines = writedLines;
            for (int resBmpY = workWritedLines, workY = 0; resBmpY < *bitmapheight && workY <
                                                                                      rowPerStrip; /*wj++,*/ workY++/*= inSampleSize*/) {
                ...
                if (globalLineCounter % inSampleSize == 0) {
                    for (int resBmpX = 0, workX = 0;
                         resBmpX < *bitmapwidth; resBmpX++, workX += inSampleSize) {
                        ...
                        if (workX + 1 < origwidth && workY - 1 >= 0) {
                            ...
                            alpha += colorMask & crPix >> 24;
                            sum++;
                        } else if (workX + 1 < origwidth && workY - 1 == -1 &&
                                   globalLineCounter > 0) {
                            crPix = matrixTopLine[workX + 1];
                            red += colorMask & crPix >> 16;
                            green += colorMask & crPix >> 8;
                        ...
                        if (workX + 1 < origwidth && workY + 1 < rowPerStrip) {
                            ...
                            alpha += colorMask & crPix >> 24;
                            sum++;
                        } else if (workX + 1 < origwidth && workY + 1 == rowPerStrip &&
                                   isSecondRasterExist) {
                        ...
                        if (workX - 1 >= 0 && workY + 1 < rowPerStrip) {
                            ...
                            alpha += colorMask & crPix >> 24;
                            sum++;
                        } else if (workX - 1 >= 0 && workY + 1 == rowPerStrip &&
                                   isSecondRasterExist) {
                            crPix = matrixBottomLine[workX - 1];
                            red += colorMask & crPix >> 16;
                            green += colorMask & crPix >> 8;
                ...
                if (workY == rowPerStrip - 1 && i + rowPerStrip < origheight) {
                    _TIFFmemcpy(matrixTopLine, raster + workY * origwidth,
                                sizeof(unsigned int) * origwidth);
                }
    ...
    } else if (origorientation == 2 || origorientation == 3 || origorientation == 6 ||
               origorientation == 7) {
        flipPixelsHorizontal(*bitmapwidth, *bitmapheight, pixels);
    }
...
jint *NativeDecoder::getSampledRasterFromStripWithBounds(int inSampleSize, int *bitmapwidth,
    ...
    *bitmapheight = boundHeight / inSampleSize;//origheight / inSampleSize;
    ...
    for (int i = 0; (i < stripMax * rowPerStrip || i > boundY + boundHeight); i += rowPerStrip) {
        ...
        int workWritedLines = writedLines;
        for (int resBmpY = workWritedLines, workY = 0;
             resBmpY < *bitmapheight && workY < rowPerStrip; /*wj++,*/ workY++/*= inSampleSize*/) {
    ...
    if (useOrientationTag &&
        (origorientation == ORIENTATION_TOPRIGHT || origorientation == ORIENTATION_BOTRIGHT
        ...
        for (int ox = startPosX, nx = 0; nx < boundWidth / inSampleSize; ox--, nx++) {
            ...
            for (int oy = 0, ny = 0; ny < boundHeight / inSampleSize; oy++, ny++) {
                if (useOrientationTag && (origorientation > 4)) {
                    tmpPixels[nx * (boundHeight / inSampleSize) + ny] = pixels[ox * *bitmapheight +
                                                                               oy];
                } else {
                    tmpPixels[ny * (boundWidth / inSampleSize) + nx] = pixels[oy * *bitmapwidth +
                                                                              ox];
                }
            }
        }
...
void NativeDecoder::rotateTileLinesVertical(uint32 tileHeight, uint32 tileWidth, uint32 *whatRotate,
                                            uint32 *bufferLine) {
...
void
NativeDecoder::rotateTileLinesHorizontal(uint32 tileHeight, uint32 tileWidth, uint32 *whatRotate,
                                         uint32 *bufferLine) {
    uint32 buf;
        ...
        for (column = 0; column < origwidth; column += tileWidth) {
            ...
            if (rightTileExists) {
                switch (origorientation) {
                    case 1:
                    case 5:
                        rotateTileLinesVertical(tileHeight, tileWidth, rasterTileRight,
                                                work_line_buf);
                        break;
                    case 2:
                    case 6:
                        rotateTileLinesVertical(tileHeight, tileWidth, rasterTileRight,
                                                work_line_buf);
                        rotateTileLinesHorizontal(tileHeight, tileWidth, rasterTileRight,
                                                  work_line_buf);
                        break;
                    case 3:
                    case 7:
            ...
            if (inSampleSize > 1) {
                ...
                for (int origTileY = 0, pixY = row / inSampleSize;
                     origTileY < tileHeight && pixY < *bitmapheight; origTileY++) {
                    ...
                    if (tileStartDataY != -1 && globalProcessedY % inSampleSize != 0) {
                        ...
                        for (int origTileX = 0, pixX = column / inSampleSize;
                             origTileX < tileWidth && pixX < *bitmapwidth; origTileX++) {
                            if (tileStartDataX != -1 && globalProcessedX % inSampleSize != 0) {
                                ...
                                if (rasterTile[srcPosition] != 0) {
                                    ...
                                    if (origTileX - 1 >= 0 && origTileY - 1 >= 0) {
                                        ...
                                        crPix = rasterTileLeft[(origTileY - 1) * tileWidth +
                                                               tileWidth - 1];
                                        if (crPix != 0) {
                                            red += colorMask & crPix >> 16;
                                            green += colorMask & crPix >> 8;
                                    ...
                                    if (origTileX - 1 >= 0) {
                                        ...
                                        crPix = rasterTileLeft[origTileY * tileWidth + tileWidth -
                                                               1];
                                        if (crPix != 0) {
                                            red += colorMask & crPix >> 16;
                                            green += colorMask & crPix >> 8;
...
jint *NativeDecoder::getSampledRasterFromTileWithBounds(int inSampleSize, int *bitmapwidth,
    ...
    jint *pixels = NULL;
    *bitmapwidth = /*boundWidth*/
            (lastTileX - firstTileX) * tileWidth / inSampleSize;//origwidth / inSampleSize;
    *bitmapheight = /*boundHeight*/
    ...
    for (row = firstTileY * tileHeight;
         row < lastTileY * tileHeight; row += tileHeight, progressRow += tileHeight) {
        columnDest = 0;
        short leftTileExists = 0;
        short rightTileExists = 0;
        for (column = firstTileX * tileWidth;
             column < lastTileX * tileWidth; column += tileWidth, progressColumn += tileWidth) {
            ...
            for (int origTileY = 0, pixY = rowDest / inSampleSize;
                 origTileY < tileHeight && pixY < *bitmapheight; origTileY++) {
                if (checkStop()) {
                    if (rasterTile) {
                        _TIFFfree(rasterTile);
                ...
                if (tileStartDataY != -1 && globalProcessedY % inSampleSize != 0) {
                    ...
                    for (int origTileX = 0, pixX = columnDest / inSampleSize;
                         origTileX < tileWidth && pixX < *bitmapwidth; origTileX++) {
                        ...
                        if (tileStartDataX != -1 && globalProcessedX % inSampleSize != 0) {
                            ...
                            if (rasterTile[srcPosition] != 0) {
                                ...
                                if (origTileX - 1 >= 0 && origTileY - 1 >= 0) {
                                    ...
                                    crPix = rasterTileLeft[(origTileY - 1) * tileWidth + tileWidth -
                                                           1];
                                    if (crPix != 0) {
                                        red += colorMask & crPix >> 16;
                                        green += colorMask & crPix >> 8;
    ...
    if (origorientation <= 4) {
        ...
        jint *tmpPixels = (jint *) malloc(sizeof(jint) * tmpPixelBufferSize);
        uint32 startPosX = boundX % tileWidth /
                           inSampleSize;//(firstTileX * tileWidth - tileWidth + boundX) / inSampleSize;
        uint32 startPosY = boundY % tileHeight /
                           inSampleSize;//(firstTileY * tileHeight - tileHeight + boundY) /inSampleSize;
        for (int ox = startPosX, nx = 0; nx < boundWidth / inSampleSize; ox++, nx++) {
...
jint *
NativeDecoder::getSampledRasterFromImage(int inSampleSize, int *bitmapwidth, int *bitmapheight) {
    //init signal handler for catch SIGSEGV error that could be raised in libtiff
    struct sigaction act;
    memset(&act, 0, sizeof(act));
    sigemptyset(&act.sa_mask);
...
jint *NativeDecoder::getSampledRasterFromImageWithBounds(int inSampleSize, int *bitmapwidth,
                                                         int *bitmapheight) {
    //init signal handler for catch SIGSEGV error that could be raised in libtiff
    struct sigaction act;
    memset(&act, 0, sizeof(act));
...
void NativeDecoder::rotateRaster(jint *raster, int angle, int *width, int *height) {
    ...
    jint *rotated = (jint *) malloc(
            sizeof(jint) * rotatedWidth * rotatedHeight);//new int[rotatedWidth * rotatedHeight];
...
void NativeDecoder::fixOrientation(jint *pixels, uint32 pixelsBufferSize, int bitmapwidth,
                                   int bitmapheight) {
    if (origorientation > 4) {
...
void NativeDecoder::writeDataToOptions(int directoryNumber) {
    ...
    */
    ...
    jfieldID gOptions_outXResolutionFieldID = env->GetFieldID(jBitmapOptionsClass, "outXResolution",
                                                              "F");
    ...
    jfieldID gOptions_outYResolutionFieldID = env->GetFieldID(jBitmapOptionsClass, "outYResolution",
                                                              "F");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outBitPerSampleFieldID = env->GetFieldID(jBitmapOptionsClass,
                                                                   "outBitsPerSample", "I");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outSamplePerPixelFieldID = env->GetFieldID(jBitmapOptionsClass,
                                                                     "outSamplePerPixel", "I");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outTileWidthFieldID = env->GetFieldID(jBitmapOptionsClass, "outTileWidth",
                                                                "I");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outTileHeightFieldID = env->GetFieldID(jBitmapOptionsClass,
                                                                 "outTileHeight", "I");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outRowPerStripFieldID = env->GetFieldID(jBitmapOptionsClass,
                                                                  "outRowPerStrip", "I");
    ...
    uint32 stripSize = TIFFStripSize(image);
    LOGII("strip size", stripSize);
    jfieldID gOptions_outStripSizeFieldID = env->GetFieldID(jBitmapOptionsClass, "outStripSize",
                                                            "I");
    ...
    jfieldID gOptions_outStripMaxFieldID = env->GetFieldID(jBitmapOptionsClass, "outNumberOfStrips",
                                                           "I");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outAuthorFieldId = env->GetFieldID(jBitmapOptionsClass, "outAuthor",
                                                             "Ljava/lang/String;");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outCopyrightFieldId = env->GetFieldID(jBitmapOptionsClass, "outCopyright",
                                                                "Ljava/lang/String;");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outimgDescrFieldId = env->GetFieldID(jBitmapOptionsClass,
                                                               "outImageDescription",
                                                               "Ljava/lang/String;");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outsoftwareFieldId = env->GetFieldID(jBitmapOptionsClass, "outSoftware",
                                                               "Ljava/lang/String;");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outdatetimeFieldId = env->GetFieldID(jBitmapOptionsClass, "outDatetime",
                                                               "Ljava/lang/String;");
    ...
    if (tagRead == 1) {
        ...
        jfieldID gOptions_outhostFieldId = env->GetFieldID(jBitmapOptionsClass, "outHostComputer",
                                                           "Ljava/lang/String;");
