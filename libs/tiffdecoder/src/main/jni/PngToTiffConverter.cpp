...
PngToTiffConverter::PngToTiffConverter(JNIEnv *e, jclass clazz, jstring in, jstring out,
...
PngToTiffConverter::PngToTiffConverter(JNIEnv *e, jclass clazz, jint in, jint out, jobject opts,
                                       jobject listener)
        : BaseTiffConverter(e, clazz, in, out, opts, listener) {
    png_ptr_init = 0;
    png_info_init = 0;
...
jboolean PngToTiffConverter::convert() {
    ...
    estimateMem += (compressionInt == COMPRESSION_CCITTRLE ||
                    compressionInt == COMPRESSION_CCITTFAX3 ||
                    compressionInt == COMPRESSION_CCITTFAX4) ? (width / 8 + 0.5) * rowPerStrip : 0;
    ...
    if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
        ...
        for (int y = 0; y < height; y += rowPerStrip) {
            ...
            TIFFWriteEncodedStrip(tiffImage, y / rowPerStrip, bilevel,
...
unsigned char *
PngToTiffConverter::convertArgbToBilevel(png_bytep *data, int samplePerPixel, uint32 width,
                                         uint32 height) {
    unsigned char red;
    unsigned char green;
    unsigned char blue;
