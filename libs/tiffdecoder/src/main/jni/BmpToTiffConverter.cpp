...
BmpToTiffConverter::BmpToTiffConverter(JNIEnv *e, jclass clazz, jstring in, jstring out,
                                       jobject opts, jobject listener)
        : BaseTiffConverter(e, clazz, in, out, opts, listener) {
}

BmpToTiffConverter::BmpToTiffConverter(JNIEnv *e, jclass clazz, jint in, jint out, jobject opts,
                                       jobject listener)
        : BaseTiffConverter(e, clazz, in, out, opts, listener) {
}
...
jboolean BmpToTiffConverter::convert() {
    ...
    if (inf->biBitCount != 16 && inf->biBitCount != 24 && inf->biBitCount != 32 &&
        inf->biBitCount != 0) {
    ...
    unsigned long estimateMem = rowPerStrip * width * 4 *
                                2;//need 2 buffers for read data from bitmap. Check getPixelsFromBmp method

    if (compressionInt == COMPRESSION_JPEG) {
        estimateMem += width * sizeof(uint32);//temp array for writing JPEG lines
        estimateMem += width *
                       sizeof(uint32);//temp array for fliping bitmap data in getPixelsFromBmp method
    ...
    if (compressionInt == COMPRESSION_CCITTRLE || compressionInt == COMPRESSION_CCITTFAX3 ||
        ...
        for (int y = 0; y < height; y += rowPerStrip) {
            ...
            free(pixels);
            ret = TIFFWriteEncodedStrip(tiffImage, y / rowToRead, bilevel,
                                        compressedWidth * sizeof(unsigned char) * rowToRead);
            free(bilevel);
        }
        ...
        for (int y = 0; y < height; y += rowPerStrip) {
            ...
            sendProgress(y * width, total);
            uint32 *pixels = getPixelsFromBmp(y, rowToRead);
            TIFFWriteEncodedStrip(tiffImage, y / rowPerStrip, pixels,
                                  width * sizeof(uint32) * rowToRead);
            free(pixels);
        }
    }
...
uint32 *BmpToTiffConverter::getPixelsFrom16Bmp(int offset, int limit) {
    ...
    lseek(inFd, bmp->bfOffBits + (height - offset - limit) * (width * 2 + (width * 2) % 4),
          SEEK_SET);
    ...
    for (i = 0; i < numImgBytes; i += 2) {
        ...
        pixels[ind] = (r << 3) | (g << greenShiftBits << 8) | (b << 3 << 16) |
                      a << 24;//(r| g << 8 | b << 16 | a << 24) ;
