...
DataPool::create(const GP<DataPool> &pool, int start, int length) {
    DEBUG_MSG("DataPool::DataPool: pool=" << (void *) ((DataPool *) pool) << " start=" << start
                                          << " length= " << length << "\n");
    DEBUG_MAKE_INDENT(3);
...
DataPool::has_data(int dstart, int dlength) {
    ...
    return (pool ? (pool->has_data(start + dstart, dlength))
                 : ((furl.is_local_file_url()) ? (start + dstart + dlength <= length)
                                               : ((dlength < 0) ? is_eof()
                                                                : (
                                                          block_list->get_bytes(dstart, dlength) ==
                                                          dlength))));
}
...
PoolByteStream::write(const void *buffer, size_t size) {
    G_THROW(ERR_MSG(
            "not_implemented_n") "\tPoolByteStream::write()");   //  PoolByteStream::write() is not implemented.
