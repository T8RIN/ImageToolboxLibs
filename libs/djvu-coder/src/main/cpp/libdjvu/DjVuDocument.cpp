    ...
    if (!url.is_empty()) {
        ...
        if (init_data_pool) {
            if (!init_url.is_empty() && init_url.is_local_file_url() && djvu_import_codec) {
                djvu_import_codec(init_data_pool, init_url, needs_compression_flag,
                                  needs_rename_flag);
            }
    ...
    GURL url;
    {
        ...
        if (url.is_empty()) {
            ...
            if (!port || !port->inherits("DjVuFile")) {
                ...
                GP<DjVuFile> file =
                        DjVuFile::create(url, const_cast<DjVuDocument *>(this), recover_errors,
                                         verbose_eof);
                ufile->file = file;
                return file;
            ...
            G_CATCH(ex)
                {
                    ...
                    G_TRY {
                        ...
                        GUTF8String emsg =
                                ERR_MSG("DjVuDocument.exclude_page") "\t" + GUTF8String(i + 1);
