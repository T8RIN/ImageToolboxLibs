...
ByteStream::read(void *buffer, size_t sz) {
    G_THROW(ERR_MSG(
            "ByteStream.cant_read"));      //  Cannot read from a ByteStream created for writing
    return 0;
}
...
ByteStream::write(const void *buffer, size_t sz) {
    G_THROW(ERR_MSG(
            "ByteStream.cant_write"));      //  Cannot write from a ByteStream created for reading
    return 0;
}
...
ByteStream::Memory::seek(long offset, int whence, bool nothrow) {
    ...
    switch (whence) {
        ...
        case SEEK_END:
            ...
            break;
        default:
            G_THROW(ERR_MSG("bad_arg") "\tByteStream::Memory::seek()");
    }
    nwhere += offset;
    if (nwhere < 0)
...
ByteStream::Static::seek(long offset, int whence, bool nothrow) {
    ...
    switch (whence) {
        ...
        case SEEK_END:
            ...
            break;
        default:
            G_THROW("bad_arg\tByteStream::Static::seek()");
