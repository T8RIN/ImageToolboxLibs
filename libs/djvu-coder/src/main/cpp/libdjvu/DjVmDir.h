...
public:
    ...
    /** File type. Possible file types are:
       ...
       \end{description} */
    enum FILE_TYPE {
        INCLUDE = 0, PAGE = 1, THUMBNAILS = 2, SHARED_ANNO = 3
    };
protected:
    /** Default constructor. */
    File(void);
...
public:
    ...
    bool is_shared_anno(void) const { return (flags & TYPE_MASK) == SHARED_ANNO; }
...
private:
    ...
    enum FLAGS_0 {
        IS_PAGE_0 = 1, HAS_NAME_0 = 2, HAS_TITLE_0 = 4
    };
    enum FLAGS_1 {
        HAS_NAME = 0x80, HAS_TITLE = 0x40, TYPE_MASK = 0x3f
    };
    unsigned char flags;
    int page_num;
};
...
DjVmDir::File::get_load_name(void) const { return id; }

inline const GUTF8String &
DjVmDir::File::get_title() const { return *(title.length() ? &title : &id); }
