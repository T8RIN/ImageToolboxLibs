...
public:
    ...
    GMonitorLock(GMonitor *gsec) : gsec(gsec) { if (gsec) gsec->enter(); };

    /** Destructor. Leaves the associated monitor. */
    ~GMonitorLock() { if (gsec) gsec->leave(); };
};
...
public:
    void lock() { GMonitor::enter(); };

    void unlock() { GMonitor::leave(); };
};
...
public:
    ...
    void wait(int timeout) {
        enter();
        if (!status) GMonitor::wait(timeout);
        status = 0;
        leave();
    };
};
...
public:
    GCriticalSectionLock(GCriticalSection *gsec) : gsec(gsec) { if (gsec) gsec->lock(); };

    ~GCriticalSectionLock() { if (gsec) gsec->unlock(); };
};
