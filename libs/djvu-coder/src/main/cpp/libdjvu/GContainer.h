...
protected:
    ...
    public:
        ...
        static void *lea(void *base, int n) { return (void *) (((char *) base) + SZ * n); }

        // Trivial default constructor
        static void init(void *dst, int n) {}

        // Trivial copy constructor
        static void copy(void *dst, const void *src, int n, int) { ::memcpy(dst, src, SZ * n); }
    ...
    public:
        ...
        // Offset in an array of T
        static void *lea(void *base, int n) { return (void *) (((T *) base) + n); }
    ...
    template<class K>
    struct SetNode : public HNode {
        K key;
    };
    // -- Class for map nodes with everything
    template<class K, class T>
    struct MapNode : public SetNode<K> {
        T val;
    };
#if !GCONTAINER_NO_MEMBER_TEMPLATES
};
...
public:
    ...
    operator TYPE *() { return ((TYPE *) data) - minlo; }
    ...
    /** Extends the subscript range so that it contains #n#.
        ...
        \end{verbatim} */
    void touch(int n) { if (n < lobound || n > hibound) GArrayBase::touch(n); }
    ...
    void resize(int hibound) { GArrayBase::resize(0, hibound); }
    ...
    void resize(int lobound, int hibound) { GArrayBase::resize(lobound, hibound); }
    ...
    void shift(int disp) { GArrayBase::shift(disp); }
    ...
    void del(int n, int howmany = 1) { GArrayBase::del(n, howmany); }
    ...
    void ins(int n, int howmany = 1) { GArrayBase::ins(n, 0, howmany); }
    ...
    void steal(GArrayTemplate &ga) { GArrayBase::steal(ga); }
    // -- SORTING
...
GArrayTemplate<TYPE>::sort(int lo, int hi) {
    ...
    if (data[hi] <= tmp) { pivot = tmp; }
    else if (data[hi] <= pivot) { pivot = data[hi]; }
    // -- partition set
    int h = hi;
    int l = lo;
    while (l < h) {
...
public:
    ...
    GPosition firstpos() const { return GListImpl<TI>::firstpos(); }

    /** Returns the last position in the list. See \Ref{GPosition}. */
    GPosition lastpos() const { return GListImpl<TI>::lastpos(); }

    /** Implicit notation for GList::firstpos(). */
    operator GPosition() const { return firstpos(); }
    ...
    /** Returns a reference to the list element at position #pos#.  This
        ...
        positions. */
    TYPE &operator[](GPosition pos) { return (TYPE &) (((LNode *) pos.check((void *) this))->val); }
    ...
    bool isempty() const { return this->nelem == 0; }
    ...
    /** Returns the position #pos# of the #n#-th list element.  An invalid
        ...
        #n#-th element. */
    GPosition nth(unsigned int n) const { return GListImpl<TI>::nth(n); }

    /*  Compatibility */
    int nth(unsigned int n, GPosition &pos) const {
        GPosition npos = nth(n);
        if (npos) pos = npos;
        return !!pos;
    }
    ...
    GPosition contains(const TYPE &elt) const {
        GPosition pos;
        GListImpl<TI>::search((const TI &) elt, pos);
        return pos;
    }
    ...
    void empty() { GListImpl<TI>::empty(); }

    /** Inserts an element after the last element of the list.
        The new element is initialized with a copy of argument #elt#. */
    void append(const TYPE &elt) { GListImpl<TI>::append(this->newnode((const TI &) elt)); }

    /** Inserts an element before the first element of the list.
        The new element is initialized with a copy of argument #elt#. */
    void prepend(const TYPE &elt) { GListImpl<TI>::prepend(this->newnode((const TI &) elt)); }
    ...
    void insert_before(GPosition pos, GListTemplate<TYPE, TI> &fromlist,
                       GPosition &frompos) { GListImpl<TI>::insert_before(pos, fromlist, frompos); }

    /** Destroys the list element at position #pos#.  This function does
        nothing unless position #pos# is a valid position. */
    void del(GPosition &pos) { GListImpl<TI>::del(pos); }
...
public:
    GPosition contains(const K &key) const { return GPosition(get(key), (void *) this); }

    void del(const K &key) { deletenode(get(key)); }
};
...
public:
    ...
    int size() const { return this->nelems; }

    /** Returns the first position in the map. */
    GPosition firstpos() const { return GMapImpl<KTYPE, TI>::firstpos(); }
    ...
    bool isempty() const { return this->nelems == 0; }
    ...
    /** Searches an entry for key #key#.  If the map contains an entry whose key
        ...
        position. */
    GPosition contains(const KTYPE &key) const { return GMapImpl<KTYPE, TI>::contains(key); }
    ...
    void empty() { GMapImpl<KTYPE, TI>::empty(); }
    ...
    void del(GPosition &pos) { GSetBase::del(pos); }

    /** Destroys the map entry for key #key#.
        Nothing is done if there is no entry for key #key#. */
    void del(const KTYPE &key) { GMapImpl<KTYPE, TI>::del(key); }
    /* Old iterators. Do not use. */
#if GCONTAINER_OLD_ITERATORS
