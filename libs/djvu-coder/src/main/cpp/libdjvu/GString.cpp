...
GUTF8String
GBaseString::getNative2UTF8(void) const { //MBCS cvt
...
/*MBCS*/
GNativeString
GBaseString::getUTF82Native(EscapeMode escape) const { //MBCS cvt
...
GStringRep::substr(const char *s, const int start, const int len) const {
    GP<GStringRep> retval;
    if (s && s[0]) {
        const unsigned int length = (start < 0 || len < 0) ? (unsigned int) strlen(s)
                                                           : (unsigned int) (-1);
    ...
    while ((amp_locn = search('&', start_locn)) > -1) {
        ...
        if (len) {
            ...
            if (s[0] == '#') {
                ...
                if (map_entry) {                           // Found in the conversion map, substitute
