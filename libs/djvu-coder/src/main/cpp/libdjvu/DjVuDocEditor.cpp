...
// Moreover it will strip out any INCL chunks in that file...
{
    ...
    if (file_pool && file_url && DjVuDocument::djvu_import_codec) {
        (*DjVuDocument::djvu_import_codec)(file_pool, file_url, needs_compression_flag,
                                           can_compress_flag);
    }
                ...
                if (!is_page) {
                    ...
                    for (GPosition pos = list; pos; ++pos) {
                        ...
                        if (list[pos]->is_include()
                            ...
                            const GP<IFFByteStream> giff_old(
                                    IFFByteStream::create(pool->get_stream()));
                            const GP<IFFByteStream> giff_new(
                                    IFFByteStream::create(file_pool->get_stream()));
                            file = 0;
...
DjVuDocEditor::save_as(const GURL &where, bool bundled) {
    ...
    if (needs_compression()) {
        ...
        mbs->flush();
        mbs->seek(0, SEEK_SET);
        djvu_compress_codec(mbs, save_doc_url,
                            (!(const DjVmDir *) djvm_dir) || (djvm_dir->get_files_num() == 1) ||
                            (save_doc_type != INDIRECT));
        files_map.empty();
        doc_url = GURL();
    } else {
