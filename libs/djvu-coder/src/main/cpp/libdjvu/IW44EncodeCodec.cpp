...
public:
    ...
    int encode_prepare(int band, int fbucket, int nbucket, IW44Image::Block &blk,
                       IW44Image::Block &eblk);
    ...
    while (again && scale < w && scale < h) {
        ...
        for (i = 0; i < h; i += scale, cp += w * scale, q += w * scale, p += rowsize * scale)
            for (j = 0; j < w; j += scale) {
                ...
                // compute gray level
                for (ii = istart;
                     ii < i + scale && ii < h; ii += split, cpp += w * split, qq += w * split)
    ...
    for (int scale = begin; scale < end; scale <<= 1) {
        ...
        for (i = 0; i < h; i += scale + scale) {
            ...
            for (j = 0; j < w; j += scale + scale)
                if (m[j] && m0[j] && m1[j] && (j <= 0 || m[j - scale]) &&
                    (j + scale >= w || m[j + scale]))
                    m[j] = 1;
                else
                    m[j] = 0;
...
int
IW44Image::Codec::Encode::encode_prepare(int band, int fbucket, int nbucket, IW44Image::Block &blk,
                                         IW44Image::Block &eblk) {
    int bbstate = 0;
...
IWBitmap::Encode::encode_chunk(GP<ByteStream> gbs, const IWEncoderParms &parm) {
    ...
    if (cserial == 0)
        cbytes += sizeof(struct IW44Image::SecondaryHeader) +
                  sizeof(struct IW44Image::TertiaryHeader);
