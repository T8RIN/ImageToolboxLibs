...
class ApngDecoder(
    ...
    suspend fun decodeApng(
        context: Context
    ): Result<Drawable> = kotlin.runCatching {
        withContext(Dispatchers.IO) {
            ...
            if (Utils.isPng(bytes)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    ...
                    drawable!!
                }.toBitmap().flexibleResize(config.getMaxSize()).let {
    ...
    fun decodeAsync(
        context: Context,
        onGetFrame: suspend (Bitmap) -> Unit
        ...
        withContext(Dispatchers.IO) {
            ...
            if (Utils.isPng(bytes)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    ...
                    drawable!!
                }.toBitmap().flexibleResize(config.getMaxSize()).also {
                    onGetFrame(it)
                }
            }
        }
