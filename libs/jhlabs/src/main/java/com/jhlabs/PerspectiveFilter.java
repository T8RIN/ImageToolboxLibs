...
public class PerspectiveFilter extends TransformFilter {
    ...
    protected void transformSpace(Rect rect) {
        if (scaled) {
            ...
            rect.top = (int) Math.min(Math.min(y0, y1), Math.min(y2, y3));
            rect.right = (int) Math.max(Math.max(x0, x1), Math.max(x2, x3)) - rect.left;
            rect.bottom = (int) Math.max(Math.max(y0, y1), Math.max(y2, y3)) - rect.top;
            return;
        }
        if (!clip) {
            float w = (float) rect.width(), h = (float) rect.height();
            Rect r = new Rect();
            PointF p1 = getPoint2D(new PointF(0, 0), null);
    ...
    public PointF getPoint2D(PointF srcPt, PointF dstPt) {
        ...
        dstPt.set(((x * a11 + y * a21 + a31) * f), ((x * a12 + y * a22 + a32) * f));
        return dstPt;
    }
