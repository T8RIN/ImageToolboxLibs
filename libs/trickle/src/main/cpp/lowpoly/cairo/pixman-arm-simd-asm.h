...
0
.
set FLAG_BRANCH_OVER,
2
.
set FLAG_PROCESS_PRESERVES_PSR,
0
.
set FLAG_PROCESS_CORRUPTS_PSR,
4
.
set FLAG_PROCESS_DOESNT_STORE,
0
.
set FLAG_PROCESS_DOES_STORE,
8 /* usually because it needs to conditionally skip it */
.
set FLAG_NO_SPILL_LINE_VARS,
0
.
set FLAG_SPILL_LINE_VARS_WIDE,
16
.
set FLAG_SPILL_LINE_VARS_NON_WIDE,
32
.
set FLAG_SPILL_LINE_VARS,
48
.
set FLAG_PROCESS_CORRUPTS_SCRATCH,
0
.
set FLAG_PROCESS_PRESERVES_SCRATCH,
64
.
set FLAG_PROCESS_PRESERVES_WK0,
0
.
set FLAG_PROCESS_CORRUPTS_WK0,
128 /* if possible, use the specified register(s) instead so WK0 can hold number of leading pixels */
.
set FLAG_PRELOAD_DST,
0
.
set FLAG_NO_PRELOAD_DST,
256
...
macro pixldst
op,
cond = al, numbytes, reg0, reg1, reg2, reg3, base, unaligned = 0
        .
if \numbytes == 16
.if \unaligned == 1
\op\()r\()\
...
.else
\op\()mia\()\cond  \base!, {
WK\()\reg0,WK\()\reg1,WK\()\reg2,WK\()\reg3}
.endif
.elseif \numbytes == 8
.if \unaligned == 1
\op\()r\()\
...
.
macro pixst_baseupdated
cond, numbytes, reg0, reg1, reg2, reg3, base
.if \numbytes == 16
stm\()\cond\()db \base, {
WK\()\reg0,WK\()\reg1,WK\()\reg2,WK\()\reg3}
.elseif \numbytes == 8
...
.if \bpp/dst_w_bpp == 4
PF add, SCRATCH,
\base, WK0, lsl #\bpp_shift-
dst_bpp_shift
        PF
and,    SCRATCH, SCRATCH, #31
PF rsb, SCRATCH, SCRATCH, WK0, lsl
#\bpp_shift-
dst_bpp_shift
        PF
sub,    SCRATCH, SCRATCH, #1        /* so now ranges are -16..-1 / 0..31 / 32..63 */
PF movs, SCRATCH, SCRATCH, lsl
...
dst_bpp_shift
        PF
rsbs,   SCRATCH, SCRATCH, WK0, lsl #32-5+\bpp_shift-
dst_bpp_shift
        PF
bls,    61f
.endif
...
.
macro preload_middle
bpp, base, scratch_holds_offset
.if \bpp > 0
...
PF  and,    WK1, \base, #31
PF add, WK1, WK1, WK0, lsl
#\
bpp_shift
        PF
add,    WK1, WK1, #32*(\
bpp *pix_per_block
/256-1)*(prefetch_distance+1)
PF bic, SCRATCH,
\base, #31
80:
PF pld,
[SCRATCH, #32*(prefetch_distance+1)]
PF add, SCRATCH, SCRATCH,
#32
PF subs, WK1, WK1,
#32
PF bhi,
...
PF mov, SCRATCH,
\base, lsl #32-5
PF adds, SCRATCH, SCRATCH, X, lsl
#32-5+\
bpp_shift
        PF
adcseq, SCRATCH, SCRATCH, #0
...
81f
PF pld,
[SCRATCH, #32*(prefetch_distance+2)]
81:
PF pld,
[SCRATCH, #32*(prefetch_distance+1)]
82:
...
.
macro preload_line
narrow_case, bpp, bpp_shift, base
...
.
macro conditional_process1_helper
cond, process_head, process_tail, numbytes, firstreg, unaligned_src, unaligned_mask, decrementx
...
.
macro conditional_process2
test, cond1, cond2, process_head, process_tail, numbytes1, numbytes2, firstreg1, firstreg2, unaligned_src, unaligned_mask, decrementx
...
FLAG_PROCESS_CORRUPTS_WK0
        movs
SCRATCH, X, lsl #32-1  /* C,N = bits 1,0 of DST */
.else
...
FLAG_PROCESS_CORRUPTS_WK0
        movs
SCRATCH, X, lsl #32-3  /* C,N = bits 3, 2 of DST */
.else
movs SCRATCH, WK0, lsl
...
dst_bpp_shift
        str
X, [sp, #LINE_SAVED_REG_COUNT*4]
mov X, WK0
.endif
/* Use unaligned loads in all cases for simplicity */
.if dst_w_bpp == 8
conditional_process2 test_bits_1_0_ptr, mi, cs,
\process_head, \process_tail, 1, 2, 1, 2, 1, 1, DECREMENT_X
.
elseif dst_w_bpp
== 16
test_bits_1_0_ptr
        conditional_process1
cs, \process_head, \process_tail, 2, 2, 1, 1, DECREMENT_X
.
endif
        conditional_process2
test_bits_3_2_ptr, mi, cs, \process_head, \process_tail, 4, 8, 1, 2, 1, 1, DECREMENT_X
.if (flags) &
FLAG_PROCESS_CORRUPTS_WK0
        ldr
X, [sp, #LINE_SAVED_REG_COUNT*4]
...
.
macro trailing_15bytes
process_head, process_tail, unaligned_src,
unaligned_mask
        conditional_process2
test_bits_3_2_pix, cs, mi, \process_head, \process_tail, 8, 4, 0, 2, \unaligned_src, \unaligned_mask, 0
.if dst_w_bpp == 16
test_bits_1_0_pix
        conditional_process1
cs, \process_head, \process_tail, 2, 0, \unaligned_src, \unaligned_mask, 0
.
elseif dst_w_bpp
== 8
conditional_process2 test_bits_1_0_pix, cs, mi,
\process_head, \process_tail, 2, 1, 0, 1, \unaligned_src, \unaligned_mask, 0
.endif
.endm


.
macro wide_case_inner_loop
process_head, process_tail, unaligned_src, unaligned_mask, dst_alignment
110:
.
set SUBBLOCK,
...
.
elseif (src_bpp
== 0) && (mask_bpp > 0) && ((flags) & FLAG_PROCESS_PRESERVES_SCRATCH)
...
PF pld,
[DST, #32*prefetch_distance - \dst_alignment]
.endif
\process_tail  , 16, 0
.if !((flags) & FLAG_PROCESS_DOES_STORE)
pixst   , 16, 0, DST
...
.
macro wide_case_inner_loop_and_trailing_pixels
process_head, process_tail, process_inner_loop, exit_label, unaligned_src, unaligned_mask
...
.if (
src_bpp *pix_per_block
> 256) || (
mask_bpp *pix_per_block
> 256) || (
dst_r_bpp *pix_per_block
> 256)
PF  and,    WK0, X, #pix_per_block-1
.
endif
        preload_trailing
src_bpp, src_bpp_shift,
SRC
        preload_trailing
mask_bpp, mask_bpp_shift, MASK
.if ((flags) & FLAG_NO_PRELOAD_DST) == 0
preload_trailing dst_r_bpp, dst_bpp_shift, DST
.
endif
        add
X, X, #(prefetch_distance+2)*pix_per_block - 128/
dst_w_bpp
/* The remainder of the line is handled identically to the medium case */
medium_case_inner_loop_and_trailing_pixels
\process_head, \process_tail,, \exit_label, \unaligned_src, \unaligned_mask
.endm

.
macro medium_case_inner_loop_and_trailing_pixels
process_head, process_tail, unused, exit_label, unaligned_src, unaligned_mask
120:
...
exit_label
        trailing_15bytes
\process_head, \process_tail, \unaligned_src, \unaligned_mask
.endm

.
macro narrow_case_inner_loop_and_trailing_pixels
process_head, process_tail, unused, exit_label, unaligned_src,
unaligned_mask
        tst
X, #16*8/
dst_w_bpp
        conditional_process1
ne, \process_head, \process_tail, 16, 0, \unaligned_src, \unaligned_mask, 0
/* Trailing pixels */
...
.
macro end_of_line
restore_x, vars_spilled, loop_label, last_one
.if \vars_spilled
...
X
.
req r0  /* pixels to go on this line */
Y           .
req r1  /* lines to go */
DST         .
req r2  /* destination pixel pointer */
STRIDE_D    .
req r3  /* destination stride (bytes, minus width) */
SRC         .
req r4  /* source pixel pointer */
STRIDE_S    .
req r5  /* source stride (bytes, minus width) */
MASK        .
req r6  /* mask pixel pointer (if applicable) */
STRIDE_M    .
req r7  /* mask stride (bytes, minus width) */
WK0         .
req r8  /* pixel data registers */
WK1         .
req r9
WK2         .
req r10
...
req r12
ORIG_W      .
req r14 /* width (pixels) */

push    {
r4-r11, lr}        /* save all registers */
...
ldr STRIDE_S,
[sp, #ARGS_STACK_OFFSET+4]
.endif
.if mask_bpp > 0
ldr MASK,
[sp, #ARGS_STACK_OFFSET+8]
ldr STRIDE_M,
[sp, #ARGS_STACK_OFFSET+12]
.endif
...
sp, sp, #4
.
set ARGS_STACK_OFFSET, ARGS_STACK_OFFSET
+4
.
set LOCALS_STACK_OFFSET, LOCALS_STACK_OFFSET
+4
.
endif
...
sub
STRIDE_D, STRIDE_D, X, lsl #dst_bpp_shift
.if src_bpp > 0
lsl STRIDE_S,
#
src_bpp_shift
        sub
STRIDE_S, STRIDE_S, X, lsl #src_bpp_shift
.endif
.if mask_bpp > 0
lsl STRIDE_M,
#
...
newline
        preload_line
0, src_bpp, src_bpp_shift,
SRC  /* in: X, corrupts: WK0-WK1 */
preload_line
0, mask_bpp, mask_bpp_shift, MASK
.if ((flags) & FLAG_NO_PRELOAD_DST) == 0
...
.
        ...
        sub
X, X, #128/
dst_w_bpp     /* simplifies inner loop termination */
ands
WK0, DST, #15
beq     164f
rsb WK0, WK0,
...
newline
        preload_line
1, src_bpp, src_bpp_shift,
SRC  /* in: X, corrupts: WK0-WK1 */
preload_line
1, mask_bpp, mask_bpp_shift, MASK
.if ((flags) & FLAG_NO_PRELOAD_DST) == 0
...
.
macro line_saved_regs
x:vararg
.
set LINE_SAVED_REGS,
0
.
set LINE_SAVED_REG_COUNT,
0
.
irp SAVED_REG,
\x
.ifc "SAVED_REG","Y"
.
set LINE_SAVED_REGS, LINE_SAVED_REGS
| (1<<1)
.
set LINE_SAVED_REG_COUNT, LINE_SAVED_REG_COUNT
+ 1
.endif
