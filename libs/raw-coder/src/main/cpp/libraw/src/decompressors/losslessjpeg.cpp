...
LibRaw_LjpegDecompressor::LibRaw_LjpegDecompressor(uint8_t *b, unsigned bs, bool dngbug, bool csfix)
        : buffer(b, bs),
...
void HuffTable::initval(uint32_t _bits[17], uint32_t _huffval[256], bool _dng_bug) {
    ...
    for (uint8_t len = 0; len < nbits; len++) {
        for (uint32_t i = 0; i < bits[len + 1]; i++) {
            for (int j = 0; j < (1 << (nbits - len - 1)); j++) {
                ...
                hufftable[h] = ((len + 1) << 16) | (uint8_t(huffval[pos] & 0xff)
    ...
    if (!disable_cache) {
        ...
        for (uint32_t i = 0; i < 1 << LIBRAW_DECODE_CACHE_BITS; i++) {
            ...
            if (pump.valid() >= 0)
                decodecache[i] = LIBRAW_CACHE_PRESENT_FLAG | uint64_t(((len & 0xff)
                        << 16) | uint16_t(val16));
        }
    }
    initialized = true;
