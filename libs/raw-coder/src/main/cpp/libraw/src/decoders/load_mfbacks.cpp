...
int LibRaw::phase_one_correct() {
    ...
    try {
        while (entries--) {
            ...
            if (tag == 0x0400) { /* Sensor defects */
            ...
            } else if (tag == 0x0419) { /* Polynomial curve - output calibraion */
                for (get4(), i = 0; i < 8; i++)
            ...
            } else if (tag == 0x041a) { /* Polynomial curve */
                for (i = 0; i < 4; i++)
                    poly[i] = getrealf(LIBRAW_EXIFTAG_TYPE_FLOAT);
                for (i = 0; i < 0x10000; i++) {
                    for (num = 0, j = 4; j--;)
            ...
            } else if (tag == 0x0401) { /* All-color flat fields - luma calibration*/
                phase_one_flat_field(1, 2);
            } else if (tag == 0x0416 || tag == 0x0410) {
                // 0x410 - luma calibration
                phase_one_flat_field(0, 2);
            } else if (tag == 0x040b) { /* Red+blue flat field - croma calibration */
                ...
                qlin_applied = 1;
            } else if (tag == 0x041e && !qmult_applied) { /* Quadrant multipliers - output calibraion */
                ...
                qmult_applied = 1;
                    ...
                    && ph1.split_row > 0 && ph1.split_row < raw_height) { /* Quadrant combined - four tile gain calibration */
...
void LibRaw::leaf_hdr_load_raw() {
    ...
    try {
        FORC(tiff_samples)for (r = 0; r < raw_height; r++) {
                checkCancel();
                if (r % tile_length == 0) {
