...
struct pana8_base_t {
    ...
    pana8_base_t(const pana8_base_t &s) {
        clone(s.coeff);
    }

    void clone(const uint32_t *scoeff) { // TODO: implement SSE load for SSE-enabled code
        coeff[0] = scoeff[0];
        coeff[1] = scoeff[1];
        coeff[2] = scoeff[2];
        coeff[3] = scoeff[3];
        ...
        for (uint32_t current_row = 0; current_row < halfheight; current_row++) {
            ...
            for (int32_t col = 0; col < doublewidth; col++) {
                ...
                if (bitportion < 0) {
                    ...
                    pixbits = (inputqword_next >> bitportion) | (inputqword
                            << (64 - (uint8_t) (bitportion & 0xffu)));
                ...
                int32_t v39 = (uint16_t) ((uint64_t) v38 >> ((uint8_t) v37 - (uint8_t) huff_index))
                        << ((huff_coeff[huff_index] >> 24) & 0xffu);
...
#undef ZERO
    ...
    for (int i = 0; i < 17; i++)
        huff_coeff[i] = (uint32_t(meta.tag41[i]) << 24) | (uint32_t(meta.tag40a[i])
                << 16) | meta.tag40b[i];
