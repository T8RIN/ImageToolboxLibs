            ...
            for (row = 0; row < height; row += (jh.clrs >> 1) - 1) {
                ...
                for (col = scol; col < ecol; col += 2, jcol += jh.clrs) {
                    ...
                    if (imgdata.rawparams.specials & LIBRAW_RAWSPECIAL_SRAW_NO_INTERPOLATE) {
                        ...
                        FORC(jh.clrs - 2)ip[col + (c >> 1) * width + (c & 1)][0] = rp[jcol + c];
                        ip[col][1] = rp[jcol + jh.clrs - 2] - 8192;
                        ip[col][2] = rp[jcol + jh.clrs - 1] - 8192;
                    } else {
                        FORC(jh.clrs - 2)ip[col + (c >> 1) * width + (c & 1)][0] = rp[jcol + c];
...
void LibRaw::pentax_load_raw() {
    ...
    FORC(dep)for (i = bit[0][c]; i <= ((bit[0][c] + (4096 >> bit[1][c]) - 1) & 4095);)
            huff[++i] = bit[1][c] << 8 | c;
    huff[0] = 12;
...
void LibRaw::quicktake_100_load_raw() {
    ...
    for (row = 2; row < height + 2; row++) {
        ...
        for (col = 2 + (row & 1); col < width + 2; col += 2) {
            val = ((pixel[(row - 1) * 644 + col - 1] + 2 * pixel[(row - 1) * 644 + col + 1] + pixel[row * 644 + col - 2])
                    >> 2) + gstep[getbits(4)];
    ...
    for (rb = 0; rb < 2; rb++)
        for (row = 2 + rb; row < height + 2; row += 2) {
            ...
            for (col = 3 - (row & 1); col < width + 2; col += 2) {
                ...
                val = ((pixel[(row - 2) * 644 + col] + pixel[row * 644 + col - 2])
                        >> 1) + rstep[sharp][getbits(2)];
                pixel[row * 644 + col] = val = LIM(val, 0, 255);
                if (row < 4)
    ...
    for (row = 2; row < height + 2; row++) {
        checkCancel();
        for (col = 3 - (row & 1); col < width + 2; col += 2) {
            val = ((pixel[row * 644 + col - 1] + (pixel[row * 644 + col]
                    << 2) + pixel[row * 644 + col + 1]) >> 1) - 0x100;
        ...
        for (col = 0; col < raw_width; col += 16) {
            ...
            for (c = 0; c < 16; c += 2) {
                ...
                if (idest < maxpixels &&
                    ...
                    RAW(row, col + c) = (i > 0 ? ((signed) ph1_bits(i) << (32 - i)
                                                                       >> (32 - i)) : 0) +
