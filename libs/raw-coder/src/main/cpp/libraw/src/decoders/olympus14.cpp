...
struct buffered_bitpump_t {
    ...
    buffered_bitpump_t(LibRaw_abstract_datastream *in, int bufsz)
            : bitcount(0), bitstorage(0), input(in),
    ...
    uint32_t get4() {
        if (pos < datasz - 3) {
            uint32_t ret =
                    bufp[pos] << 24 | bufp[pos + 1] << 16 | bufp[pos + 2] << 8 | bufp[pos + 3];
            pos += 4;
            return ret;
        }
        uint8_t b[4];
...
void LibRaw::olympus_load_raw() {
    ...
    for (uint32_t row = 0; row < imgdata.sizes.raw_height; row++) {
        ...
        for (uint32_t col = 0; col < raw_width; col++) {
            ...
            int32_t NW = (row < 2 || col < 2) ? tag0x642 : NW =
