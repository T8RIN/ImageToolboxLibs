...
#define wbuffer(i) ((unsigned short)buffer[lastoffset + 15 - i])
    pixelbuffer[0] = (wbuffer(0) << 6) | (wbuffer(1)
            >> 2);                                         // 14 bit
    pixelbuffer[1] = (((wbuffer(1) & 0x3) << 12) | (wbuffer(2) << 4) | (wbuffer(3)
            >> 4)) & 0x3fff; // 14 bit
    pixelbuffer[2] = (wbuffer(3)
            >> 2) & 0x3;                                                       // 2
    pixelbuffer[3] = ((wbuffer(3) & 0x3)
            << 8) | wbuffer(4);                                        // 10
    pixelbuffer[4] = (wbuffer(5) << 2) | (wbuffer(6)
            >> 6);                                         // 10
    pixelbuffer[5] = ((wbuffer(6) & 0x3f) << 4) | (wbuffer(7)
            >> 4);                                // 10
...
#define wb(i) ((unsigned short)buffer[lastoffset + 15 - i])
    ...
    pixelbuffer[2] = (wb(3)
            >> 6) & 0x3;                       // 2; 2u/3, 6 low bits remains in wb(3)
    pixelbuffer[3] = ((wb(3) & 0x3f) << 2) | (wb(4)
            >> 6);     // 8; 6l/3 + 2u/4; 6 low bits remains in wb(4)
    pixelbuffer[4] = ((wb(4) & 0x3f) << 2) | (wb(5)
            >> 6);     // 8: 6l/4 + 2u/5; 6 low bits remains in wb(5)
    pixelbuffer[5] = ((wb(5) & 0x3f) << 2) | (wb(6)
            >> 6);     // 8: 6l/5 + 2u/6, 6 low bits remains in wb(6)

    pixelbuffer[6] = (wb(6) >> 4) & 0x3;                       // 2, 4 low bits remains in wb(6)
    pixelbuffer[7] = ((wb(6) & 0xf) << 4) | (wb(7)
            >> 4);      // 8: 4 low bits from wb(6), 4 upper bits from wb(7)
    pixelbuffer[8] = ((wb(7) & 0xf) << 4) | (wb(8)
            >> 4);      // 8: 4 low bits from wb7, 4 upper bits from wb8
    pixelbuffer[9] = ((wb(8) & 0xf) << 4) | (wb(9)
            >> 4);      // 8: 4 low bits from wb8, 4 upper bits from wb9

    pixelbuffer[10] = (wb(9)
            >> 2) & 0x3;                      // 2: bits 2-3 from wb9, two low bits remain in wb9
    pixelbuffer[11] = ((wb(9) & 0x3) << 6) | (wb(10)
            >> 2);    // 8: 2 bits from wb9, 6 bits from wb10
    pixelbuffer[12] = ((wb(10) & 0x3) << 6) | (wb(11)
            >> 2);   // 8: 2 bits from wb10, 6 bits from wb11
    pixelbuffer[13] = ((wb(11) & 0x3) << 6) | (wb(12)
            >> 2);   // 8: 2 bits from wb11, 6 bits from wb12
...
struct iiq_bitstream_t {
    ...
    iiq_bitstream_t(uint32_t *img_input, size_t inpsz)
            : curr(0), input(img_input), used(0), pos(0), maxpos(inpsz) {
    }
...
void decode_S_type(int32_t out_width, uint32_t *img_input, ushort *outbuf, size_t datalen /*, int bit_depth*/) {
    ...
    if (out_width - 7 > 0) {
        ...
        for (int blk_id = 0; blk_id < block_count; ++blk_id) {
            ...
            int pix_sub[2] = {
                    0xFFFF >> (pix_sub_init - bit_check[0]),
                    0xFFFF >> (pix_sub_init - bit_check[1])};
            ...
            for (int i = 0; i < 8; i++) // MAIN LOOP for pixel decoding
            {
                ...
                if (bit_check[i & 1] == 9)
                    ...
                    value = prev_pix_value[i & 1] + ((uint32_t) stream.get(take_bits)
                            << bp_shift[i & 1]) - pix_sub[i & 1];
...
struct p1_row_info_t {
    ...
    bool operator<(const p1_row_info_t &rhs) const {
    ...
    for (int row = 0; row < (int) imgdata.sizes.raw_height; row++) {
        ...
        for (int col = 0; col < (int) imgdata.sizes.raw_width - 15; col += 16) {
                ...
                for (int i = 0; i < 16; i++) {
                    ...
                    RAW(row, col + i) =
                            motion_average[motion] ?
                                    (RAW(refrow, refcol) + RAW(refrow, refcol + 2) + 1)
                                            >> 1 : RAW(refrow, refcol);
                }
            }
            ...
            for (int i = 0; i < 16; i++) {
                ...
                int rcol = (row % 2) ? col + ((i & 0x7) << 1) + 1 - (i >> 3) : col + ((i & 0x7)
                        << 1) + (i >> 3);
