...
int LibRaw::simplify_make_model(unsigned *_maker_index,
    ...
    unsigned mkindex = 0;
    for (int i = 0; i < int(sizeof CorpTable / sizeof *CorpTable); i++) {
        if (strcasestr(_make, CorpTable[i].CorpName)) { /* Simplify company names */
...
#define COPYARR(to, from) memmove(&to, &from, sizeof(from))
        ...
        if (iifd < (int) tiff_nifds && iifd >= 0) {
                    ...
                    if (colidx >= 0) // Selected
                    {
                        ...
                        if (calidx[colidx] == sidx) {
                            for (int i = 0; i < colors && i < 4; i++)
                                FORCC cc[i][c] = tiff_ifd[sidx].dng_color[colidx].calibration[i][c];
                        }
            ...
            if (sidx >= 0 && tiff_ifd[sidx].dng_levels.dng_whitelevel[0])
                ...
                FORC4 imgdata.color.dng_levels.dng_whitelevel[c] = (1 << tiff_ifd[iifd].bps) - 1;
            ...
            if (sidx >= 0) {
                ...
                if (imgdata.color.dng_levels.asshotneutral[0]) {
                    cam_mul[3] = 0;
                    FORCC if (fabs(imgdata.color.dng_levels.asshotneutral[c]) > 0.0001)
...
void LibRaw::identify_finetune_by_filesize(INT64 fsize) {
    ...
    if (fsize == 4771840) { // hack Nikon 3mpix: E880, E885, E990, E995;
        // Olympus C-3030Z
        if (!timestamp && nikon_e995())
            strcpy(model, "E995");
    } else if (fsize == 2940928) { // hack Nikon 2mpix: E2100, E2500
...
//         raw_height = 30848;
        ...
        if ((imHassy.SensorCode == 4) && !strncmp(model, "V96C", 4)) { // Hasselblad V96C
            ...
            top_margin = 108;
            height = raw_height - top_margin;
        } else if ((imHassy.SensorCode == 20) && imHassy.uncropped) { // Hasselblad X2D-100c, CFV-100c
            left_margin = 124;
            width = 11664;
            top_margin = 92;
            height = raw_height - top_margin;
