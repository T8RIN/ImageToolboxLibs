            ...
            while (relpos_inBox + 8 < relpos_inDir + szItem) {
                ...
                if (Tag == 0x927c) {
                    ...
                    if (callbacks.exif_cb) {
                        ...
                        callbacks.exif_cb(callbacks.exifparser_data, (tItem
                                        << 20) | 0x80000 | 0x927c, 7, lTag - 8, order, ifp,
    ...
    */
    ...
    while (((oAtom + 8ULL) <= (oAtomList + szAtomList)) && ((oAtom + 8ULL) < filesz)) {
        ...
        if (szAtom == 0ULL) {
            ...
            uint64_t upper = get4();
            uint64_t lower = get4();
            szAtom = ((upper & 0x7fffffff)
                    << 32) | lower; // This will limit atom size to 2^63-1. In practice, you can live with this
            if (szAtom < 16ULL) {
                err = -3;
                goto fin;
            }
                ...
                if (!memcmp(xdata + 12, "PRVW", 4)) {
                    ...
                    if (imgdata.thumbs_list.thumbcount < LIBRAW_THUMBNAIL_MAXCOUNT) {
                        ...
                        if (do_add) {
                            ...
                            imgdata.thumbs_list.thumblist[idx].tlength = thumb_length;
                            imgdata.thumbs_list.thumblist[idx].tflip = 0xffff;
                            imgdata.thumbs_list.thumblist[idx].tmisc = (3
                                    << 5) | 8; // 3 samples/8 bps
                            imgdata.thumbs_list.thumblist[idx].twidth = (xdata[22]
                                    << 8) + xdata[23];
                            imgdata.thumbs_list.thumblist[idx].theight = (xdata[24]
                                    << 8) + xdata[25];
                            imgdata.thumbs_list.thumbcount++;
                        }
                    }
