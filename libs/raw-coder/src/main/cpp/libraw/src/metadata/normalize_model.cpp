...
//    }
    ...
    if (makeIs(LIBRAW_CAMERAMAKER_Canon)) {
        ...
        for (i = 0; i < int(sizeof phase1alias / sizeof *phase1alias); i++) { // re-badged Phase One backs
        ...
        for (i = 0; i < int(sizeof leafalias / sizeof *leafalias); i++) { // re-badged Leaf backs
        ...
        for (i = 0; i < int(sizeof leafalias / sizeof *leafalias); i++) { // re-badged Leaf backs
            if (leafalias[i][0] == '@') {
                orig = leafalias[i] + 1;
                if (!strcmp(model, orig)) break;
            } else if (!strcmp(model, leafalias[i])) { // maybe to change regular "make" to "Mamiya" too
                strcpy(normalized_model, orig);
                break;
            }
        }
        ...
        if (makeIs(LIBRAW_CAMERAMAKER_Canon)) {
            ...
            if (!strncmp(normalized_model, "DC-S", 4)) {
                ...
                if (!strncmp(normalized_model + 2, "-LX100", 6) || // DC-LX100M2
                        !strncmp(normalized_model + 3, "-LX100", 6)) { // DMC-LX100
            ...
            if (!strncmp(normalized_model, "fp", 2)) {
                ...
                } else if (normalized_model[11] == 'H') { // 'sd Quattro H'
                    ilm.CameraFormat = LIBRAW_FORMAT_SigmaAPSH;
                } else if (normalized_model[4] == 'Q') { // 'sd Quattro'
                    ilm.CameraFormat = LIBRAW_FORMAT_APSC;
                } else {
            ...
            if (!strncmp(model, "KD-", 3)) { // Konica KD-400Z, KD-510Z
                ilm.CameraMount = LIBRAW_MOUNT_FixedLens;
            }
        } else if (makeIs(LIBRAW_CAMERAMAKER_Mamiya)) {
