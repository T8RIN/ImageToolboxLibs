...
#define CR3_ColorData(offset)                                \
  ...
  offsetWhiteLevels        = save1 + ((offset+0x02d5) << 1);
    ...
    } else if (tag == 0x0095 && !ilm.Lens[0]) { // lens model tag
    ...
    } else if (tag == 0x00e0) // SensorInfo
    {
        ...
        if (offsetChannelBlackLevel) {
            ...
            FORC4 bls += (imCanon.ChannelBlackLevel[RGGB_2_RGBG(c)] = get2());
        ...
        if (offsetWhiteLevels) {
            ...
            FORC4 imgdata.color.linear_max[c] = imCanon.SpecularWhiteLevel;
        }
