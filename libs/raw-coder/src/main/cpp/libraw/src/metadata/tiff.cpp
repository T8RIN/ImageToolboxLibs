...
int LibRaw::parse_tiff_ifd(INT64 base) {
    ...
    while (entries--) {
        ...
        if (!is_pana_raw) { /* processing of EXIF tags that collide w/ PanasonicRaw tags */
            switch (tag) {
                case 0x0001:
                    if (len == 4)
                        is_pana_raw = get4();
        ...
        } else { /* processing Panasonic-specific "PanasonicRaw" tags */
            switch (tag) {
        ...
        switch (tag) {            /* processing of general EXIF tags */
...
//printf ("wb start in DNG: 0x%04x\n", fj*2-0x4e);
            ...
            case 0xc61a: /* 50714, BlackLevel */
                        ...
                        tiff_ifd[ifd].samples == (int) len) // LinearDNG, per-channel black
                {
                    ...
                    if (tag == 0xc61a) {
                        tiff_ifd[ifd].dng_levels.parsedfields |= LIBRAW_DNGFM_BLACK;
                        FORC(int(cblack[4] *cblack[5]))tiff_ifd[ifd].dng_levels.dng_cblack[6 + c] = cblack[6 + c];
                        ...
                        if (!strncmp(mbuf, "MakN", 4)) {
                            ...
                            if (!strncmp(mbuf, "SR2 ", 4)) {
                                ...
                                if (SR2SubIFDLength && (SR2SubIFDLength < 10240000) &&
                                        (buf_SR2 = (unsigned *) calloc(SR2SubIFDLength + 1024, 1))) { // 1024b for safety
        ...
        if (i != raw &&
            ...
            if (imgdata.thumbs_list.thumbcount < LIBRAW_THUMBNAIL_MAXCOUNT && tiff_ifd[i].bytes > 0) {
                ...
                if (!already) {
                    ...
                    imgdata.thumbs_list.thumblist[idx].tmisc = tiff_ifd[i].bps | (
                            tiff_ifd[i].samples << 5);
