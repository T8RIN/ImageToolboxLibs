...
public:
    ...
    ushort sget2LL(INT64 offset) {
        return sget2(int(offset));
    }
    ...
    unsigned sget4LL(INT64 offset) {
        return sget4(int(offset));
    }

    double sgetreal(int type, int offset);

    float sgetrealf(int type, int offset) {
        return float(sgetreal(type, offset));
    }
