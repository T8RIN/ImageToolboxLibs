...
struct BitPumpJpeg : public BitPump {
    ...
    uint32_t peek(uint32_t num) {
        if (num > nbits && !finished) {
            if ((size >= 4) && pos < size && buffer[pos] != 0xff && buffer[pos + 1] != 0xff && buffer[pos + 2] != 0xff &&
                    buffer[pos + 3] != 0xff) {
                uint64_t inbits = (uint32_t(buffer[pos]) << 24) | (uint32_t(buffer[pos + 1])
                        << 16) |
    ...
    BitPumpJpeg(ByteStreamBE &s)
            : buffer(s.buffer + s.pos),
              size(s.size - s.pos),
              pos(0),
              bits(0),
              nbits(0),
...
struct LibRaw_JpegComponentInfo {
    ...
    LibRaw_JpegComponentInfo(const LibRaw_JpegComponentInfo &q)
            : id(q.id), index(q.index), dc_tbl(q.dc_tbl),
