...
int readJ2KHeader(opj_codec_t *l_codec, int decod_format, image_header_t *outHeader) {
    ...
    outHeader->hasAlpha = (header->m_private_image->numcomps == 2 ||
                           header->m_private_image->numcomps == 4);
...
int
decodeJP2Header(opj_stream_t *l_stream, opj_dparameters_t *parameters, image_header_t *outHeader) {
...
int decodeJP2Stream(opj_stream_t *l_stream, opj_dparameters_t *parameters, image_data_t *outImage,
                    jint reduce) {
    opj_codec_t *l_codec = NULL;                /* Handle to a decompressor */
    opj_image_t *image = NULL;
    ...
    if (readJ2KHeader(l_codec, parameters->decod_format, &outHeader) == EXIT_SUCCESS) {
        if (parameters->cp_reduce >= outHeader.numResolutions) {
            LOGE("The number of resolutions to remove (%d) is greater or equal than the number of resolutions of this image (%d). Changing to %d.",
                 parameters->cp_reduce, outHeader.numResolutions, outHeader.numResolutions - 1);
        ...
        if (parameters->cp_layer > 0 && parameters->cp_layer > outHeader.numQualityLayers) {
             ...
             */
            LOGE("The number of quality layers to decode (%d) is greater than the number of quality layers of this image (%d). Changing to %d.",
                 parameters->cp_layer, outHeader.numQualityLayers, outHeader.numQualityLayers);
        }
    }
...
static OPJ_SIZE_T opj_read_from_byte_array(void *p_buffer, OPJ_SIZE_T p_nb_bytes,
                                           opj_byte_array_source *p_user_data) {
...
static OPJ_SIZE_T opj_write_from_byte_array(void *p_buffer, OPJ_SIZE_T p_nb_bytes,
                                            opj_byte_array_source *p_user_data) {
    ...
    while (p_nb_bytes + p_user_data->offset > p_user_data->availableLength) {
        ...
        char *newBuffer = (char *) realloc(p_user_data->data, p_user_data->availableLength +
                                                              BYTE_ARRAY_SRC_CHUNK_LENGTH);
...
static OPJ_OFF_T
opj_skip_from_byte_array(OPJ_OFF_T p_nb_bytes, opj_byte_array_source *p_user_data) {
...
opj_stream_t *OPJ_CALLCONV opj_stream_create_byte_array_stream(char *data, unsigned int length,
    ...
    opj_byte_array_source *data_src = (opj_byte_array_source *) malloc(
            sizeof(opj_byte_array_source));
    data_src->data = data;
    data_src->offset = 0;
...
int setEncoderParameters(opj_cparameters_t *parameters, JNIEnv *env, jint fileFormat,
                         jint numResolutions, jfloatArray compressionRates,
    ...
    if (compressionRates) {
        ...
        if (dataLength > 100)
            dataLength = 100; //opj_cparameters supports maximum of 100 quality layers
        if (dataLength > 0) {
...
opj_image_t *getImage(JNIEnv *env, int *pixels, jboolean hasAlpha, jint width, jint height,
    ...
    image->x1 = !image->x0 ? (width - 1) * parameters->subsampling_dx + 1 : image->x0 +
                                                                            (width - 1) *
                                                                            parameters->subsampling_dx +
                                                                            1;
    image->y1 = !image->y0 ? (height - 1) * parameters->subsampling_dy + 1 : image->y0 +
                                                                             (height - 1) *
...
int
encodeJP2(opj_cparameters_t *parameters, opj_image_t *image, opj_byte_array_source **outByteArray) {
    int i, j;
    ...
    if (parameters->outfile[0] != 0) {
        /* open a byte stream for writing and allocate memory for all tiles */
        l_stream = (opj_stream_private_t *) opj_stream_create_default_file_stream(
                parameters->outfile, OPJ_FALSE);
    } else {
        l_stream = (opj_stream_private_t *) opj_stream_create_byte_array_stream(NULL, 0,
...
Java_com_gemalto_jp2_JP2Decoder_decodeJP2ByteArray(JNIEnv *env, jclass thiz, jbyteArray data,
    ...
    l_stream = opj_stream_create_byte_array_stream(imgData, dataLength, OPJ_J2K_STREAM_CHUNK_SIZE,
                                                   1);
    if (!l_stream) {
...
Java_com_gemalto_jp2_JP2Decoder_readJP2HeaderFile(JNIEnv *env, jclass thiz, jstring fileName) {
    opj_stream_t *l_stream = NULL;                /* Stream */
    opj_dparameters_t parameters;            /* decompression parameters */
...
JNIEXPORT jintArray JNICALL
Java_com_gemalto_jp2_JP2Decoder_readJP2HeaderByteArray(JNIEnv *env, jclass thiz, jbyteArray data) {
    opj_stream_t *l_stream = NULL;                /* Stream */
    opj_dparameters_t parameters;            /* decompression parameters */
    char *imgData;
    jbyte *bufferPtr;
