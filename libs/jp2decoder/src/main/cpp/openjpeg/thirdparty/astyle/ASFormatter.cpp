 ...
 */
    string ASFormatter::nextLine() {
        ...
        while (!isLineReady) {
                ...
                if (shouldIndentPreprocBlock
                        ...
                        preproc.substr(0, 2) == "if") // #if, #ifdef, #ifndef
                    {
                        if (isImmediatelyPostPreprocessor)
                            breakLine();
                        isIndentableProprocessorBlock = isIndentablePreprocessorBlock(currentLine,
                                                                                      charNum);
                    ...
                    currentChar != ';') // allow ;;
                {
                    ...
                    if (isBraceType(braceTypeStack->back(), SINGLE_LINE_TYPE)) {
                        ...
                        if (isBeforeAnyLineEndComment(blockEnd)) {
                            size_t commentStart = currentLine.find_first_not_of(" \t",
                                                                                blockEnd + 1);
            ...
            if (currentChar == '{' || currentChar == '}') {
                ...
                bool isOpeningArrayBrace = (isBraceType(braceType, ARRAY_TYPE)
                                            ...
                                            && !isBraceType(
                        (*braceTypeStack)[braceTypeStack->size() - 2], ARRAY_TYPE)
                );
            ...
            if (isPotentialHeader && !isInTemplate) {
                ...
                if (isCStyle()) {
                    if (findKeyword(currentLine, charNum, AS_ASM)
                        ...
                        isInAsm = true;
                    } else if (findKeyword(currentLine, charNum,
                                           AS_MS_ASM)        // microsoft specific
        ...
        if (prependEmptyLine        // prepend a blank line before this formatted line
            ...
            enhancer->enhance(beautifiedLine, isInNamespace, isInPreprocessorBeautify,
                              isInBeautifySQL);
 ...
 */
    void ASFormatter::initNewLine() {
        ...
        for (charNum = 0;
             isWhiteSpace(currentLine[charNum]) && charNum + 1 < (int) len; charNum++) {
        ...
        } else if (isWhiteSpace(currentLine[charNum]) &&
                   !(charNum + 1 < (int) currentLine.length())) {
            lineIsEmpty = true;
        }
 ...
 */
    void ASFormatter::appendCharInsideComments() {
        if (formattedLineCommentNum ==
            string::npos     // does the comment start on the previous line?
            || formattedLineCommentNum == 0) {
    ...
    void ASFormatter::padOperators(const string *newOperator) {
        ...
        bool shouldPad = (newOperator != &AS_SCOPE_RESOLUTION
                          ...
                          && !(newOperator == &AS_COLON &&
                               !foundQuestionMark            // objC methods
                          ...
                          && !(newOperator == &AS_QUESTION            // check for Java wildcard
                               ...
                               && (previousNonWSChar == '<'
                                   ...
                                   || nextNonWSChar == '.'))
                          && !(newOperator ==
                               &AS_QUESTION            // check for C# null conditional operator
                               && isSharpStyle()
                               && (nextNonWSChar == '.'
                                   || nextNonWSChar == '['))
                          && !isCharImmediatelyPostOperator
 ...
 */
    void ASFormatter::formatPointerOrReference() {
        ...
        int itemAlignment = (currentChar == '*' || currentChar == '^') ? pa : ((ra ==
                                                                                REF_SAME_AS_PTR)
                                                                               ? pa : ra);
    ...
    void ASFormatter::formatPointerOrReferenceToMiddle() {
        ...
        if (isSequenceReached("**")) {
            ...
            for (size_t i = charNum;
                 i < currentLine.length() - 1 && isWhiteSpace(currentLine[i]); i++)
                goForward(1);
        }
        ...
        for (size_t i = charNum + 1;
             i < currentLine.length() && isWhiteSpace(currentLine[i]); i++) {
            goForward(1);
                ...
                if (i != string::npos) {
                    ...
                    if (formattedLine[i] == '{')
                        ...
                        if (prevWordH != nullptr)
                            ...
                            prevIsParenHeader = true;
                        else if (isCStyle() && prevWord == AS_THROW &&
                                 shouldPadHeader) // don't unpad
                            prevIsParenHeader = true;
                        else if (prevWord == "and" || prevWord == "or" ||
                                 prevWord == "in")  // don't unpad
                            prevIsParenHeader = true;
        ...
        } else    // attach brace
        {
            ...
            if (isCharImmediatelyPostComment || isCharImmediatelyPostLineComment) {
                if (isOkToBreakBlock(braceType)
                    && !(isCharImmediatelyPostComment &&
                         isCharImmediatelyPostLineComment)    // don't attach if two comments on the line
                    && !isImmediatelyPostPreprocessor
                    ...
                    && previousCommandChar != ';')    // don't attach ; {
                {
                    ...
                    appendCurrentChar();                // don't attach
                       ...
                       || previousCommandChar ==
                          ';')        // '}' , ';' chars added for proper handling of '{' immediately after a '}' or ';'
            {
                appendCurrentChar();                    // don't attach
            } else {
 ...
 */
    size_t
    ASFormatter::findNextChar(const string &line, char searchChar, int searchStart /*0*/) const {
    ...
    void ASFormatter::trimContinuationLine() {
        ...
        if (leadingSpaces > 0 && len > 0) {
            ...
            for (i = 0; (i < len) && (i + continuationIncrementIn < leadingSpaces); i++) {
                ...
                if (currentLine[i] == '\t')
                    continuationIncrementIn +=
                            tabSize - 1 - ((continuationIncrementIn + i) % tabSize);
            }
 ...
 */
    ...
    void ASFormatter::updateFormattedLineSplitPoints(char appendedChar) {
        ...
        if (appendedChar == '{' || appendedChar == '}'
            ...
            || nextChar == '{' || nextChar == '}'
            || currentChar == '{' ||
            currentChar == '}')    // currentChar tests for an appended brace
            return;
        ...
        if (isWhiteSpace(appendedChar)) {
            ...
            if (nextChar != ' ' && nextChar != '}' &&
                nextChar != '/')    // check for following comment
            {
                if (formattedLine.length() <= maxCodeLength)
        ...
        if (formattedLine.length() > maxCodeLength && !isLineReady) {
            ...
            if (splitPoint > 0 && splitPoint < formattedLine.length()) {
                ...
                if (maxWhiteSpacePending > 0) {
                    maxWhiteSpace = (maxWhiteSpacePending > splitPoint) ? (maxWhiteSpacePending -
                                                                           splitPoint) : 0;
            ...
            for (size_t i = 0; i < nextLine_.length(); i++) {
                ...
                if (nextLine_[i] == ':') {
                    ...
                    if (shouldPadMethodColon) {
                        ...
                        if (objCColonPadMode == COLON_PAD_ALL ||
                            objCColonPadMode == COLON_PAD_BEFORE)
                            colonAdjust = 1 - spaces;
                        else if (objCColonPadMode == COLON_PAD_NONE ||
                                 objCColonPadMode == COLON_PAD_AFTER)
...
// pad an Objective-C method colon
    void ASFormatter::padObjCMethodColon() {
        ...
        if (objCColonPadMode == COLON_PAD_NONE
            ...
            for (int i = formattedLine.length() - 1;
                 (i > -1) && isWhiteSpace(formattedLine[i]); i--) {
