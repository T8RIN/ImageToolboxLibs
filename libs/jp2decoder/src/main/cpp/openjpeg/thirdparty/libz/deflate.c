        ...
        int stream_size;
{
    ...
    strm->state = (struct internal_state FAR
    *)s;
    s->strm = strm;
        ...
        z_streamp source;
{
    ...
    *) ds;
    ...
    zmemcpy((voidpf)
    ds->prev, (voidpf)
    ss->prev, ds->w_size * sizeof(Pos));
    zmemcpy((voidpf)
    ds->head, (voidpf)
    ss->head, ds->hash_size * sizeof(Pos));
    zmemcpy(ds->pending_buf, ss->pending_buf, (uInt)
    ds->pending_buf_size);
        ...
        IPos cur_match;                             /* current match */
{
    ...
    if ((uInt)best_len <= s->lookahead) return (uInt)
    best_len;
    return s->lookahead;
}
#endif /* ASMV */
