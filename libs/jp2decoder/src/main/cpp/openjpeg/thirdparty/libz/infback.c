...
strm->
msg = Z_NULL;                 /* in case we return an error */
...
while (copy--)
state->lens[state->have++] = (unsigned short)
len;
}
}
...
state->
whave = state->wsize - left;
inflate_fast(strm, state
->wsize);
LOAD();
break;
}
